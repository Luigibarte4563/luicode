"""One-time consolidation of legacy LUICODE dotenv state."""
...
LEGACY_OPENCODE_SMOKE_MODEL_ENV = "LUICODE_SMOKE_MODEL_OPENCODE"
OPENCODE_ZEN_SMOKE_MODEL_ENV = "LUICODE_SMOKE_MODEL_OPENCODE_ZEN"

_SPECIAL_SMOKE_KEYS = frozenset(
    {
        "LUICODE_SMOKE_MODEL_MISTRAL_REASONING",
        "LUICODE_SMOKE_NIM_MODELS",
        "LUICODE_SMOKE_NIM_EXTRA_MODELS",
...
def recognized_env_keys() -> frozenset[str]:
    """Return every dotenv key LUICODE may import into managed configuration."""
...
def consolidate_managed_config(
    ...
    """Consolidate pre-schema configuration into ``~/.luicode/.env`` once."""
    ...
    if base_is_managed:
        schema = base_values.get(LUICODE_CONFIG_SCHEMA_ENV)
        ...
        if schema not in {None, ""}:
            raise ValueError(
                ...
                f"this LUICODE version supports {CONFIG_SCHEMA_VERSION}."
            )
    ...
    for key in _RETIRED_ENV_KEYS:
        values.pop(key, None)
    values.pop(LUICODE_CONFIG_SCHEMA_ENV, None)
    _materialize_auth_state(values, process, had_legacy_state=base_path is not None)
    for key in tuple(values):
        if (
            key in known
            and key != LUICODE_CONFIG_SCHEMA_ENV
            and not values[key].strip()
        ):
            values.pop(key)
    values[LUICODE_CONFIG_SCHEMA_ENV] = CONFIG_SCHEMA_VERSION
    normalized = normalize_retired_model_settings(
...
def render_managed_config(
    ...
    *,
    ...
    """Render sparse canonical LUICODE configuration."""

    known = recognized_env_keys()
    secret_keys = secret_env_keys()
    canonical = dict(values)
    canonical[LUICODE_CONFIG_SCHEMA_ENV] = CONFIG_SCHEMA_VERSION
    lines = [
        "# Managed by luicode.",
        "# Edit settings in /admin when possible.",
        f"{LUICODE_CONFIG_SCHEMA_ENV}={CONFIG_SCHEMA_VERSION}",
    ]
    configured = [
        key
        for key in canonical
        if key != LUICODE_CONFIG_SCHEMA_ENV and key in known and canonical[key].strip()
    ]
    if configured:
