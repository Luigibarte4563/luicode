...
from luicode.config.reasoning import (
...
_NON_PROVIDER_FIELDS: tuple[ConfigFieldSpec, ...] = (
    ...
    ConfigFieldSpec(
        ...
        description="Require the retained API/CLI token on LUICODE API routes.",
    ),
    ConfigFieldSpec(
    ...
    ConfigFieldSpec(
        ...
        description="Open the Admin UI after the next luicode-server launch becomes healthy.",
    ),
    ConfigFieldSpec(
        "MESSAGING_PLATFORM",
        "Messaging Platform",
    ...
    ConfigFieldSpec(
        ...
        description=(
            "Let Claude Code use WebSearch through LUICODE and allow forced local web "
...
def _catalog_smoke_fields() -> tuple[ConfigFieldSpec, ...]:
    return tuple(
        ConfigFieldSpec(
            key=f"LUICODE_SMOKE_MODEL_{provider_id.upper()}",
