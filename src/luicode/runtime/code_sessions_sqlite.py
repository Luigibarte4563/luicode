"""Relational, transaction-owned state for LUICODE coding conversations."""
...
class SQLiteCodeStore:
    ...
    def _initialize(self) -> None:
        ...
        if not self._lock.acquire():
            raise CodeUnavailableError(
                "Code sessions is already owned by another LUICODE server."
            )
        if os.name != "nt":
    ...
    def _initialize_schema(self, connection: sqlite3.Connection) -> None:
        ...
        connection.execute(
            ...
            (
                now_ms(),
                "LUICODE restarted before this turn finished. Its input was not resent.",
            ),
        )
        connection.execute(
