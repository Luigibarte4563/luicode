...
from luicode.application.errors import (
    ApplicationError,
...
from luicode.config.admin.status import provider_config_status
from luicode.config.loader import clear_settings_cache
...
from luicode.harnesses import (
    claude_desktop_integration,
    claude_integration,
    codex_integration,
    jetbrains_acp_integration,
)
from luicode.messaging.platforms import factory as messaging_platform_factory
from luicode.messaging.platforms.factory import MessagingPlatformOptions
from luicode.messaging.platforms.ports import (
    MessagingPlatformComponents,
    MessagingRuntime,
)
from luicode.messaging.voice import Transcriber
from luicode.providers.credential_validation import (
    CredentialStatus,
    check_credentials,
)
...
class ApplicationRuntime:
            ...
            async with self._lifecycle_lock:
                ...
                self._startup_tasks.append(
                    asyncio.create_task(
                        ...
                        name="luicode-retired-chat-cleanup",
                    )
                )
                if self._code_service is not None:
                    self._startup_tasks.append(
                        asyncio.create_task(
                            self._code_service.start(), name="luicode-code-startup"
                        )
                    )
                self._startup_tasks.append(
                    asyncio.create_task(
                        self._start_messaging_if_configured(),
                        name="luicode-messaging-startup",
                    )
                )
                self._started = True
    ...
    def _check_integration_available(self) -> None:
        if self._draining or self._pending_fields:
            raise ApplicationUnavailableError(
                "Wait for LUICODE to restart before changing the integration."
            )
    ...
    def _start_integration_update(
        ...
        if update.task is None or update.task.done():
            ...
            update.task = asyncio.create_task(run(), name="luicode-integration-update")
    ...
    async def _claude_desktop(self, action: IntegrationAction) -> JsonObject:
        async with self._config_lock:
            ...
            try:
                ...
                raise InvalidRequestError(
                    "Claude Desktop is managed by an organization, or its policy could not be read. LUICODE can configure only unmanaged Desktop installations."
                ) from None
                ...
                raise InvalidRequestError(
                    "Could not configure Claude Desktop. Check its configuration JSON and LUICODE disconnect record, and ensure LUICODE uses a localhost address and a nonempty managed token."
                ) from None
            except OSError:
                raise ApplicationUnavailableError(
                    "Could not access Claude Desktop settings or the LUICODE disconnect record. Fully quit Claude Desktop, check file permissions, and retry."
                ) from None
            if action in {"connect", "disconnect"}:
        ...
        try:
            ...
            def load_modules() -> None:
                for name in ("cli.managed", "messaging.session", "messaging.workflow"):
                    importlib.import_module(f"luicode.{name}")
                importlib.import_module(
                    f"luicode.messaging.platforms.{self.settings.messaging_platform}"
                )
            ...
            self._messaging_error = "Messaging could not start. Check its configuration and restart LUICODE."
    ...
    async def _start_messaging_workflow(
        ...
        import luicode.cli.managed as cli_managed
        import luicode.messaging.session as messaging_session
        import luicode.messaging.workflow as messaging_workflow_module
