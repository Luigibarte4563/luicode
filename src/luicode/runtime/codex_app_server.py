...
from luicode.application.code_sessions.models import (
    CodeCatalog,
    CodeConflictError,
    CodeMode,
    CodeModel,
...
from luicode.application.code_sessions.ports import EventSink, HarnessSelection
from luicode.application.model_catalog import CatalogModel, read_model_catalog
from luicode.application.ports import RequestRuntimePort
from luicode.cli.process_registry import register_pid, unregister_pid
from luicode.config.model_refs import split_provider_model_ref
from luicode.config.server_urls import local_proxy_root_url
from luicode.core.json_types import JsonObject
from luicode.core.version import package_version
from luicode.harnesses.codex import CODEX_INSTALL_HINT, prepare_codex_launch
...
class CodexAppServer:
    ...
    async def start(self) -> None:
        try:
            ...
            self._alive = True
            self._reader = asyncio.create_task(self._read(), name="luicode-codex-jsonl")
            self._stderr = asyncio.create_task(
                self._read_stderr(), name="luicode-codex-stderr"
            )
            self._dispatcher = asyncio.create_task(
                self._dispatch(), name="luicode-codex-events"
            )
            await self.rpc(
                "initialize",
                {
    ...
    async def create_thread(self) -> NativeThread:
        response = await self.rpc(
            "thread/start", {"cwd": self._cwd, "modelProvider": "luicode"}
        )
    ...
    async def resume_thread(self, thread_id: str) -> NativeThread:
        response = await self.rpc(
            "thread/resume",
            {"threadId": thread_id, "cwd": self._cwd, "modelProvider": "luicode"},
        )
        native = self._thread_with_permissions(response)
        self.thread_id = native.id
        return native
    ...
    async def start_turn(
        ...
        if not self.thread_id or model is None:
            raise CodeUnavailableError(
                "Codex is not prepared for the selected LUICODE model."
            )
        params: JsonObject = {
    ...
    async def rpc(self, method: str, params: JsonObject) -> JsonObject:
        ...
        request_id = f"luicode-{uuid.uuid4()}"
    ...
    async def _reap_owned_process(self) -> None:
        if self._reaping is None:
            self._reaping = asyncio.create_task(self._reap(), name="luicode-codex-reap")
