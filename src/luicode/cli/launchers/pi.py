"""Installed Pi launcher using the bundled LUICODE provider extension."""
...
def _configure(
    ...
    if not extension_path.is_file():
        raise ValueError(
            "LUICODE's bundled Pi extension is missing. Reinstall LUICODE."
        )
    return PreparedLaunch(
        ...
        client_environment(
            ...
            remove_prefixes=("LUICODE_PI_",),
            updates={
                "LUICODE_PI_BASE_URL": ctx.proxy_root_url.rstrip("/"),
                "LUICODE_PI_API_KEY": ctx.auth_token,
            },
        ),
    )
...
SPEC = HarnessSpec(
    ...
    compatibility_check=NativeCheck(
        ...
        "LUICODE requires Pi 0.80.4 or newer for provider request headers.",
    ),
)
