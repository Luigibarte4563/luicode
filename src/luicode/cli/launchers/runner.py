...
from luicode.application.model_catalog import ModelCatalog
...
def launch_harness(spec: HarnessSpec, argv: Sequence[str] | None = None) -> None:
    """Prepare the LUICODE connection and leave command semantics to the harness."""
    ...
    try:
        ...
        if not auth_token:
            raise LaunchError("luicode proxy authentication token is empty.")
        proxy_root_url = local_proxy_root_url(settings)
        if error := preflight_proxy(proxy_root_url):
            raise LaunchError(
                f"luicode proxy is not reachable at {proxy_root_url}: {error}\n"
                "Start it in another terminal with: luicode-server"
            )
        catalog = None
