...
from luicode.application.model_catalog import CatalogModel, ModelCatalog
...
def fetch_proxy_model_catalog(
    ...
    """Fetch the authenticated LUICODE-local `/v1/models` response directly."""
...
def model_catalog_from_response(payload: Mapping[str, JsonValue]) -> ModelCatalog:
    ...
    if default_model_id is None:
        raise ValueError(
            "model catalog is missing its default; restart the LUICODE server after updating"
        )
...
def catalog_models_from_response(
    ...
    """Project an LUICODE `/v1/models` response into direct client model records."""
