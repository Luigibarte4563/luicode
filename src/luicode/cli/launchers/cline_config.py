"""Process-local Cline CLI configuration for LUICODE model routing."""
...
# Cline's released session gateway only instantiates built-in providers. LUICODE
...
def build_cline_config(
    ...
    *,
    ...
    """Translate a non-empty LUICODE model snapshot into Cline's file contracts."""

    if not models:
        raise ValueError("Cline requires at least one routable LUICODE model")
    ...
    provider_settings: JsonObject = {
        ...
        "apiKey": auth_token,
        "headers": {"x-luicode-launch-id": launch_id},
