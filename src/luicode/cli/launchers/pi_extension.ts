...
const API_KEY_ENV = "LUICODE_PI_API_KEY";
const BASE_URL_ENV = "LUICODE_PI_BASE_URL";
...
export function projectLuicodeModels(payload: unknown): ProviderModelConfig[] {
	if (!isRecord(payload) || payload.object !== "list" || !Array.isArray(payload.data)) {
		throw new Error("LUICODE model catalog returned an invalid response shape.");
	}
	...
	if (models.length === 0) {
		throw new Error("LUICODE model catalog contains no routable provider models.");
	}
	return models;
}
...
async function fetchLuicodeModels(baseUrl: string, apiKey: string): Promise<ProviderModelConfig[]> {
	...
	try {
		...
		try {
			...
			if (error instanceof Error && error.name === "AbortError") {
				throw new Error(`LUICODE model catalog timed out after ${CATALOG_TIMEOUT_MS}ms.`);
			}
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Could not reach the LUICODE model catalog: ${message}`);
		}

		if (!response.ok) {
			throw new Error(`LUICODE model catalog returned HTTP ${response.status}${requestIdSuffix(response)}.`);
		}
		...
		try {
			...
			throw new Error(`LUICODE model catalog returned invalid JSON${requestIdSuffix(response)}.`);
		}
		return projectLuicodeModels(payload);
	} finally {
		clearTimeout(timeout);
	}
}

export default async function luicode(pi: ExtensionAPI): Promise<void> {
	const baseUrl = normalizeBaseUrl(requireEnvironment(BASE_URL_ENV));
	const apiKey = requireEnvironment(API_KEY_ENV);
	const models = await fetchLuicodeModels(baseUrl, apiKey);

	pi.registerProvider("luicode", {
		name: "luicode",
		baseUrl,
		apiKey: `$${API_KEY_ENV}`,
		authHeader: true,
	...
	pi.on("before_provider_request", (event, ctx) => {
		...
		// so LUICODE can choose the provider's supported thinking mode and encoding.
