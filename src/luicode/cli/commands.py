"""Implementations for installed luicode commands."""
...
from luicode.cli.local_http import open_local_request
from luicode.cli.process_registry import kill_all_best_effort
from luicode.config.loader import (
    ManagedConfigStore,
...
def _start_admin_browser(
    ...
    """Hand off an optional browser action without keeping LUICODE alive."""
    ...
    try:
        threading.Thread(
            target=open_browser, name="luicode-open-admin-browser", daemon=True
        ).start()
    except Exception as exc:
        logger.warning(
...
def serve() -> None:
    ...
    try:
        ...
        logger.error("Could not start LUICODE: {}", exc)
...
class ServerSupervisor:
    """Own one LUICODE server lifecycle, including config-driven restarts."""
    ...
    def run(
        ...
        *,
        ...
        with self._lock:
            self._run_scheduled = False
            if self._running:
                raise RuntimeError("The LUICODE server supervisor is already running.")
    ...
    def _run_bound(
        ...
        *,
        ...
        from luicode.runtime.bootstrap import build_asgi_app
...
def open_admin_when_ready(
    ...
    """Recognize an external LUICODE instance and attempt to open its local Admin page."""
