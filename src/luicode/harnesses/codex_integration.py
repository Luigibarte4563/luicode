...
from luicode.config.server_urls import proxy_v1_url, same_proxy_url
...
def _read(path: Path) -> TOMLDocument:
    ...
    provider = _child_table(providers, "luicode")
...
def _recognizes_connection(document: TOMLDocument, proxy_root_url: str) -> bool:
    provider = _child_table(_child_table(document, "model_providers"), "luicode")
    return document.get("model_provider") == "luicode" and same_proxy_url(
        provider.get("base_url"), proxy_v1_url(proxy_root_url)
    )


def recognizes_connection(path: Path, proxy_root_url: str) -> bool:
    """Identify LUICODE independently of newly introduced managed settings."""
...
def _connected(document: TOMLDocument, catalog_path: Path, proxy_root_url: str) -> bool:
    ...
    return (
        ...
        and auth.get("command") == "luicode-codex"
...
def refresh_connected(path: Path, catalog_path: Path, proxy_root_url: str) -> bool:
    """Refresh an existing LUICODE connection without selecting a provider or model."""
    path = path.resolve()
    document = _read(path)
...
def configure(
    ...
    """Inspect or edit LUICODE's entries without changing the user's selected model."""
    path = path.resolve()
    ...
    if connected is True:
        ...
        before = tomlkit.dumps(document)
        _child_table(document, "model_providers").pop("luicode", None)
        if document.get("model_provider") == "luicode":
