"""Configure a custom LUICODE agent using JetBrains' installed Claude ACP adapter."""
...
from luicode.harnesses.claude import claude_proxy_values
from luicode.harnesses.config_file import atomic_write_text

_NAME = "Claude Code (LUICODE)"
_MARKER = "LUICODE_JETBRAINS_ACP"
...
_MISSING = (
    ...
    "LUICODE supports standard local JetBrains installation locations."
)
...
def _entry(document: JsonObject) -> tuple[JsonObject, JsonObject | None]:
    ...
    if env.get(_MARKER) != "1":
        raise SetupError(
            "An unrelated agent already uses the name Claude Code (LUICODE). "
