"""Manage LUICODE's own saved Claude Desktop gateway configuration."""
...
from luicode.config.server_urls import same_proxy_url
...
class PendingMigrationError(ValueError):
    """Desktop must move its legacy Windows data before LUICODE creates the new root."""
...
class _Library:
    ...
    def profile_path(self) -> Path:
        return self.file(f"configLibrary/{LUICODE_ID}.json")
    ...
    def selected(self) -> bool:
        return (
            self.metadata is not None and self.metadata.get("appliedId") == LUICODE_ID
        )
...
def _load(root: Path) -> _Library:
    ...
    if library.metadata is not None:
        ...
        for entry in entries:
            ...
            if entry_id == LUICODE_ID and entry["name"] != "LUICODE":
                raise ValueError("LUICODE profile ID is already used")
            ids.add(entry_id)
    ...
    if library.profile is not None:
        ...
        registered = any(entry["id"] == LUICODE_ID for entry in library.entries)
...
def _disconnect(library: _Library, path: Path, intent: bool | None) -> None:
    has_entry = any(entry["id"] == LUICODE_ID for entry in library.entries)
    ...
    library = _load(library.root)
    entries = [entry for entry in library.entries if entry["id"] != LUICODE_ID]
    ...
    if remaining.profile is not None or any(
        entry["id"] == LUICODE_ID for entry in remaining.entries
    ):
...
def configure(
    ...
    *,
    ...
    if connected is True:
        ...
        entries = [entry for entry in library.entries if entry["id"] != LUICODE_ID]
        entries.append({"id": LUICODE_ID, "name": "LUICODE"})
        metadata = dict(library.metadata or {})
        metadata.update({"entries": entries, "appliedId": LUICODE_ID})
        metadata.pop("hybridPointer", None)
        _write(library.metadata_path, metadata)
