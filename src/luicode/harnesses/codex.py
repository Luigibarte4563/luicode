...
from luicode.application.model_catalog import (
    CatalogModel,
    catalog_wire_slug_for_ref,
)
from luicode.config.server_urls import proxy_v1_url
from luicode.harnesses.environment import client_environment
...
def codex_config_values(
    *, api_url: str, model: str | None = None
    ...
    """Shared configuration for the LUICODE Responses provider."""

    values: dict[str, str | list[str]] = {
        "model_provider": "luicode",
        "model_providers.luicode.name": "luicode",
        "model_providers.luicode.base_url": proxy_v1_url(api_url),
        "model_providers.luicode.auth.command": "luicode-codex",
        "model_providers.luicode.auth.args": [PRINT_PROXY_AUTH_TOKEN_FLAG],
        "model_providers.luicode.wire_api": "responses",
    }
    if model:
        values["model"] = model
    return values
...
def codex_config_args(*, api_url: str, model: str | None = None) -> list[str]:
    """Build native TOML overrides for the LUICODE Responses provider."""
