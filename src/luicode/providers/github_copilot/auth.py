"""Opt-in LUICODE connection state over native Copilot account credentials."""
...
from luicode.config.paths import github_copilot_auth_path
...
class CopilotAuthManager:
    ...
    def _invalidate(
        ...
        if self._enabled and revision == self._revision and broker is self._broker:
            if not self._disable():
                message += " LUICODE could not save the disconnected state."
    ...
    def _disable(self) -> bool:
        ...
        try:
            ...
            self._last_error = "LUICODE could not save the disconnected state. Check state-directory permissions and retry Disconnect."
            return False
        finally:
    ...
    async def _complete_login(self, attempt: str) -> None:
        ...
        try:
            ...
            if self._attempt_id == attempt and not self._closed:
                self._last_error = (
                    "Copilot sign-in exceeded LUICODE's time limit. Connect again."
                    if isinstance(error, TimeoutError)
                    else str(error)
                    if isinstance(error, CopilotUnavailable)
                    else "Copilot connection failed. Check the CLI installation, selected account and LUICODE state-directory permissions."
                )
        finally:
