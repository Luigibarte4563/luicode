...
from luicode.providers.model_listing import reasoning_capability_from_options
...
def _deny_permission(
    ...
    return PermissionDecisionReject(feedback="LUICODE owns tool execution.")
...
class SdkRuntime:
    ...
    async def start(self) -> None:
        async with self._lock:
            ...
            directory = tempfile.TemporaryDirectory(prefix="luicode-copilot-")
    ...
    async def _close_client(self) -> None:
        ...
        try:
            ...
            if self._stopped:
                ...
                try:
                    ...
                    raise CopilotUnavailable(
                        "Could not reconnect Copilot to remove an LUICODE session record. Retry disconnect to finish cleanup."
                    ) from None
        ...
        if self._sessions or self._pending_session_ids:
            raise CopilotUnavailable(
                "Copilot stopped, but an LUICODE session record could not be removed. Retry disconnect to finish cleanup."
            )
        self._client = None
        if self._directory is not None:
            self._directory.cleanup()
