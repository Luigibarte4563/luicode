...
const MASKED_SECRET = "********";
const NULL_VALUE = "__LUICODE_NULL__";
const VIEW_GROUPS = [
  {
    id: "providers",
    label: "Providers",
...
async function disconnectConnectedAccount(providerId) {
  const provider = connectedAccountDescriptor(providerId);
  if (!window.confirm(`Disconnect this ${connectedAccountName(provider)} account from LUICODE?`)) return;
  clearConnectedAccountPoll(providerId);
  try {
...
function renderField(field) {
  ...
  const input = window.LuicodeFormControls.configure(inputForField(field));
...
function createModelCombobox(input, field) {
  return new window.LuicodeModelCombobox(input, {
...
class ModelListEditor {
  constructor(input, field) {
    ...
    this.addInput = window.LuicodeFormControls.configure(document.createElement("input"));
...
async function reconnectAfterRestart() {
  ...
  try {
    ...
    if (target.origin !== window.location.origin) {
      ...
      target.hash = new URLSearchParams({ "luicode-applied": JSON.stringify(warnings) }).toString();
...
function showRestartNotice() {
  ...
  const notice = fragment.get("luicode-applied");
  if (notice === null) return;
  fragment.delete("luicode-applied");
...
async function apply(providerId = null) {
  ...
  try {
    ...
    const message = pending.length
      ? `Applied. Restart luicode-server to use: ${pending.join(", ")}`
      : "Applied";
...
try {
  for (const key of Object.keys(sessionStorage)) {
    if (key.startsWith("luicode.chat.draft.")) sessionStorage.removeItem(key);
  }
} catch {
...
function renderClaudeIntegration() {
  ...
  byId("claudeIntegrationDescription").textContent = connected
    ? "Remove LUICODE's VS Code settings. Claude onboarding stays completed."
    : "Will set LUICODE's URL and token, enable model discovery, skip VS Code login, and complete Claude onboarding.";
...
function renderCodexIntegration() {
  ...
  byId("codexIntegrationDescription").textContent = connected
    ? "Remove LUICODE's Codex configuration. Other settings stay unchanged."
    : "Configure Codex to use LUICODE. Your selected model stays unchanged.";
...
byId("confirmCodexIntegration").addEventListener("click", async () => {
  ...
  try {
    ...
    integrationMessage("codexIntegrationMessage", disconnect
      ...
      : "Settings saved. Restart Codex and select an LUICODE model.");
  } catch (error) {
...
function renderJetBrainsIntegration() {
  ...
  byId("jetBrainsIntegrationDescription").textContent = connected
    ? "Remove the Claude Code (LUICODE) agent, including its custom settings. Finish any configuration edits first."
    : "Add Claude Code (LUICODE) using JetBrains' installed Claude Agent. Start that agent once before connecting, and finish any configuration edits.";
...
byId("confirmJetBrainsIntegration").addEventListener("click", async () => {
  ...
  try {
    ...
    integrationMessage("jetBrainsIntegrationMessage", disconnect
      ...
      : "Settings saved. Reopen JetBrains, select Claude Code (LUICODE), and start a new chat.");
  } catch (error) {
...
function renderClaudeDesktopIntegration() {
  ...
  byId("claudeDesktopIntegrationDescription").textContent = disconnectPending
    ? "Finish removing LUICODE's configuration. Fully quit Claude Desktop before retrying, then reopen it."
    : connected
    ? "Remove LUICODE's configuration and return to normal Claude sign-in. Fully quit Claude Desktop first, then reopen it."
    : "Set LUICODE as Claude Desktop's gateway. Fully quit Claude Desktop before connecting, then reopen it.";
