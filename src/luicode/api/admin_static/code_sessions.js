(() => {
  ...
  function element(tag, text, className) {
    ...
    return window.LuicodeFormControls.configure(node);
  }
  ...
  function saved(id) {
    try {
      return JSON.parse(sessionStorage.getItem(`luicode.code.${id}`)) || {};
    } catch {
      return {};
    }
  }
  function save(id, value) {
    try {
      sessionStorage.setItem(`luicode.code.${id}`, JSON.stringify(value));
    } catch {
    ...
    dialog(
      ...
      (form, context) => {
        ...
        form.append(
          element(
            "p",
            "Choose an existing folder on the computer running LUICODE. You can also enter its path manually.",
            "code-notice",
          ),
        );
        return readFolder;
    ...
    dialog(
      ...
      (form) => {
        form.append(
          element(
            "p",
            "This deletes the conversation from LUICODE and Codex history. Your project files stay in place.",
          ),
        );
      },
      "Delete session",
