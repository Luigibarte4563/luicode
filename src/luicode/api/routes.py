...
async def list_models(
    ...
    x_luicode_model_view: ModelCatalogView | None = Header(default=None),
    services: ApiServices = Depends(get_services),
    _auth=Depends(require_proxy_auth),
):
    """List the model ids this proxy advertises to compatible clients."""
    trace_event(stage="ingress", event="luicode.api.models.list", source="api")
    snapshot = await services.requests.wait_for_catalog()
    return build_models_list_response(
        snapshot.settings,
        snapshot,
        view=view or x_luicode_model_view or ModelCatalogView.CLAUDE,
    )
...
async def stop_cli(
    ...
    trace_event(
        stage="ingress",
        event="luicode.api.cli.stop_via_messaging_workflow",
        source="api",
