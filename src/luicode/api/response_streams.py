...
from luicode.core.anthropic import anthropic_error_type_for_failure
...
from luicode.core.async_iterators import try_close_async_iterator
from luicode.core.diagnostics import safe_exception_message
from luicode.core.failures import find_execution_failure
    ...
    async def _close(self, *, preserved_error: BaseException | None) -> None:
        ...
        if task is None:
            task = asyncio.create_task(
                ...
                name="luicode-api-response-cleanup",
            )
...
def _trace_response_cleanup_failure(operation: str, exc: BaseException) -> None:
    trace_event(
        stage="egress",
        event="luicode.api.response.cleanup_failed",
        source="api",
        operation=operation,
...
async def _close_pre_start_body(
    ...
    *,
    ...
    task = asyncio.create_task(
        ...
        name="luicode-api-pre-start-stream-cleanup",
    )
    await _wait_for_cleanup(task)
