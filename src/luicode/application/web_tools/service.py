...
from luicode.core.anthropic.server_tool_sse import (
...
from luicode.core.web_tools import WebSearchResult
...
class WebToolService:
    ...
    def try_stream_messages(
        ...
        trace_event(
            stage="routing",
            event="luicode.api.optimization.web_server_tool",
            source="api",
    ...
    async def _stream_automatic_search(
        ...
        *,
        ...
        trace_event(
            stage="routing",
            event="luicode.api.web_search.automatic_recognized",
            source="api",
        ...
        if not tool_calls:
            trace_event(
                stage="execution",
                event="luicode.api.web_search.automatic_declined",
                source="api",
                request_id=request_id,
                model=routed.resolved.original_model,
            )
        ...
        trace_event(
            stage="execution",
            event="luicode.api.web_search.automatic_selected",
            source="api",
            request_id=request_id,
            model=routed.resolved.original_model,
        )
        ...
        trace_event(
            stage="execution",
            event="luicode.api.web_search.automatic_completed",
            source="api",
            request_id=request_id,
            model=routed.resolved.original_model,
        )
