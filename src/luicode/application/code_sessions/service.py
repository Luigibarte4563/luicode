...
from luicode.application.session_events import (
    EventPublisher,
    EventSubscription,
)
...
class CodeService:
    ...
    async def close(self) -> None:
        ...
        if self._close_task is None:
            self._close_task = asyncio.create_task(
                self._close(), name="luicode-code-close"
            )
                ...
                if owner.state.busy and not owner.storage_failed:
                    try:
                        await self._finish_locked(
                            owner,
                            "interrupted",
                            "LUICODE stopped before this turn finished.",
                        )
                    except Exception:
                        self._storage_failure(owner)
        if self._jobs:
    ...
    async def _create(self, session_id: str, cwd: str) -> CodeSession:
        ...
        try:
            ...
            raise CodeValidationError(
                "Choose an existing folder on the LUICODE computer."
            ) from None
        ...
        async with owner.lock:
            ...
            if expected_epoch != self.epoch:
                raise CodeConflictError(
                    "LUICODE restarted. Your draft has been kept; send it when you are ready."
                )
            self._editable(owner, revision)
    ...
    async def _commit_progress(
        ...
        if owner.storage_failed:
            raise CodeUnavailableError(
                "Code storage failed. Restart LUICODE to restore saved history."
            )
        try:
    ...
    def _storage_failure(self, owner: _SessionRuntime) -> None:
        ...
        self._publish(
            owner,
            "session.notice",
            message="Code storage failed. This session was stopped; restart LUICODE to restore saved history.",
        )
