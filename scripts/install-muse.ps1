...
$script:MuseOwner = "luicode-muse-installer"
...
Installs or updates LUICODE's managed native-Windows Muse Code executable.
...
function Get-MuseInstallPaths {
    ...
    return [pscustomobject] @{
        ...
        Record = Join-Path $root ".luicode-muse-install.json"
    }
}
...
function Remove-MuseTemporaryRoot {
    ...
    if (
        ...
        (-not $leafName.StartsWith("luicode-muse-", [StringComparison]::Ordinal))
    ) {
...
function Invoke-MuseInstaller {
    ...
    if (
        ...
        throw "The managed Muse Code install path exists but is not owned by LUICODE: '$($paths.Root)'. Move or remove it, then rerun the installer."
    }
    ...
    if ($null -eq $record) {
        if (
            ...
            throw "The managed Muse Code directory exists but is not owned by LUICODE: '$($paths.Root)'. Move or remove it, then rerun the installer."
        }
    ...
    $temporaryRoot = Join-Path ([IO.Path]::GetTempPath()) ("luicode-muse-" + [guid]::NewGuid().ToString("N"))
