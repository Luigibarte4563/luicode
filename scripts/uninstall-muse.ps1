...
Removes only the native-Windows Muse Code executable managed by LUICODE.
...
function Invoke-MuseUninstaller {
    ...
    if (-not $rootExists -and -not $recordExists) {
        Write-Host "No LUICODE-managed Muse Code installation was found."
        return
    }
    if (-not $recordExists) {
        throw "The Muse Code install root is not owned by LUICODE; refusing to remove '$($paths.Root)'."
    }

    [void] (Read-MuseOwnershipRecord -RecordPath $paths.Record)

    if ($DryRun) {
        Write-Host "Dry run: remove the LUICODE-managed executable and installer residue from '$($paths.Bin)'."
    ...
    $recordResidue = @(Get-ChildItem -LiteralPath $paths.Root -Force -File | Where-Object {
        $_.Name -cmatch '^\.luicode-muse-install\.json\.[0-9a-f]{32}\.(?:tmp|backup)$'
    })
    ...
    if (Test-Path -LiteralPath $paths.Root -PathType Container) {
        ...
        Write-Host "The LUICODE-managed Muse Code installation was removed."
    }
}
