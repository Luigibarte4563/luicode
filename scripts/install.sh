...
assert_no_luicode_processes_running() {
    ...
    for command_name in $LUICODE_COMMANDS; do
        process_ids=$(luicode_process_ids "$command_name")
    ...
    if [ -n "$running" ]; then
        fail "luicode is still running ($running). Stop those processes, then rerun the installer."
    fi
}
...
download_and_run() {
    ...
    temporary_file=$(mktemp "${TMPDIR:-/tmp}/luicode-install.XXXXXX") || fail "Unable to create a temporary file for $label."
...
install_rtk() {
    ...
    temporary_file=$(mktemp "${TMPDIR:-/tmp}/luicode-rtk.XXXXXX") || fail "Unable to create a temporary RTK archive."
...
assert_no_opencode_processes_running() {
    [ -z "$(luicode_process_ids opencode)$(luicode_process_ids opencode2)" ] ||
...
install_luicode() {
    ...
    if [ -n "$torch_backend" ]; then
        ...
        run uv tool install --force --refresh-package luicode --python "$PYTHON_VERSION" "$spec"
    fi
}
...
configure_and_verify_luicode() {
    ...
    if [ "$dry_run" -eq 1 ]; then
        print_command uv tool dir --bin
        printf '+ verify luicode-desktop, luicode-server, luicode-claude, luicode-codex, luicode-pi, luicode-opencode, luicode-cline, luicode-hermes, luicode-dsh, luicode-grok, luicode-muse, and luicode-aider in the uv tool bin directory\n'
        print_command luicode-server --version
        return 0
    fi

    add_uv_tool_bin_directory

    for command_name in luicode-desktop luicode-server luicode-claude luicode-codex luicode-pi luicode-opencode luicode-cline luicode-hermes luicode-dsh luicode-grok luicode-muse luicode-aider luicode-update; do
        [ -x "$tool_bin/$command_name" ] || fail "luicode installation did not create $tool_bin/$command_name."
    done

    run "$tool_bin/luicode-server" --version
}
...
install_macos_desktop_app() {
    ...
    owner_file="$contents_dir/$LUICODE_MACOS_OWNER_FILE"
    executable_dir="$contents_dir/MacOS"
    executable_path="$executable_dir/luicode-desktop"
    resources_dir="$contents_dir/Resources"
    icon_path="$resources_dir/AppIcon.icns"
    desktop_dir="$HOME/Desktop"
    desktop_link="$desktop_dir/luicode.app"

    if [ -e "$app_dir" ] || [ -L "$app_dir" ]; then
        macos_app_is_luicode_owned "$app_dir" ||
            fail "An app not managed by luicode already exists at $app_dir. Move it, then rerun the installer."
    fi

    if [ "$dry_run" -eq 1 ]; then
        print_command mkdir -p "$executable_dir" "$resources_dir" "$desktop_dir"
        print_command luicode-desktop --export-icon "$icon_path"
    ...
    run "$tool_bin/luicode-desktop" --export-icon "$icon_path"
    [ -f "$icon_path" ] || fail "luicode did not export its macOS app icon to $icon_path."
    printf '%s\n' "$LUICODE_MACOS_BUNDLE_ID" > "$owner_file"
...
<dict>
    ...
    <string>luicode</string>
    <key>CFBundleExecutable</key>
    <string>luicode-desktop</string>
    <key>CFBundleIdentifier</key>
    <string>com.luicode.desktop</string>
...
PLIST
    desktop_command=$(shell_quote "$tool_bin/luicode-desktop")
    {
        printf '%s\n' '#!/bin/sh'
        printf 'exec %s\n' "$desktop_command"
    } > "$executable_path"
    ...
    if [ -L "$desktop_link" ]; then
        if [ "$(readlink "$desktop_link")" = "$app_dir" ]; then
            ...
            printf 'A non-LUICODE link already exists at %s; leaving it unchanged.\n' "$desktop_link"
            return 0
        fi
    elif [ -e "$desktop_link" ]; then
        printf 'A non-LUICODE item already exists at %s; leaving it unchanged.\n' "$desktop_link"
        return 0
    fi
...
step "Installing or updating luicode"
install_luicode
...
if [ "$dry_run" -eq 1 ]; then
    ...
    if [ "$install_claude" -eq 1 ]; then
        printf 'Run Claude Code with: luicode-claude\n'
    fi
    if [ "$install_codex" -eq 1 ]; then
        printf 'Run Codex with: luicode-codex\n'
    fi
    if [ "$pi_available" -eq 1 ]; then
        printf 'Run Pi with: luicode-pi\n'
    fi
    if [ "$install_opencode" -eq 1 ]; then
        printf 'Run OpenCode with: luicode-opencode\n'
    fi
    if [ "$install_cline" -eq 1 ]; then
        printf 'Run Cline with: luicode-cline\n'
    else
        printf 'The luicode-cline wrapper is ready after you install Cline CLI.\n'
    fi
    if [ "$install_hermes" -eq 1 ]; then
        printf 'Run Hermes Agent with: luicode-hermes\n'
    else
        printf 'The luicode-hermes wrapper is ready after you install Hermes Agent.\n'
    fi
    if [ "$install_dsh" -eq 1 ]; then
        printf 'Run DeepSeek Harness with: luicode-dsh\n'
    else
        printf 'The luicode-dsh wrapper is ready after you install DeepSeek Harness %s.\n' "$DSH_VERSION"
    fi
    if [ "$install_grok" -eq 1 ]; then
        printf 'Run Grok Build with: luicode-grok\n'
    else
        printf 'The luicode-grok wrapper is ready after you install Grok Build.\n'
    fi
    if [ "$install_muse" -eq 1 ]; then
        printf 'Run Muse Code with: luicode-muse\n'
    else
        printf 'The luicode-muse wrapper is ready after you install Muse Code.\n'
    fi
    if [ "$install_aider" -eq 1 ]; then
        printf 'Run Aider with: luicode-aider\n'
    else
        printf 'The luicode-aider wrapper is ready after you install Aider.\n'
    fi
fi
