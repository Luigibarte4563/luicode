...
def test_luicode_server_entrypoint_starts_server(smoke_config: SmokeConfig) -> None:
    with start_server(
        smoke_config,
...
def test_claude_cli_prompt_when_available(
    ...
    with start_server(
        ...
        result = run_captured_text(
            [claude_bin, "-p", "Reply with exactly LUICODE_SMOKE_PONG"],
            cwd=tmp_path,
            env=env,
    ...
    if "LUICODE_SMOKE_PONG" not in result.stdout:
