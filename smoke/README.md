...
The second command skips everything unless `LUICODE_LIVE_SMOKE=1` is set, but still
...
```powershell
$env:LUICODE_LIVE_SMOKE = "1"
$env:LUICODE_SMOKE_TARGETS = "providers"
...
catalog/docs and can be overridden with `LUICODE_SMOKE_MODEL_<PROVIDER>`, for example
`LUICODE_SMOKE_MODEL_DEEPSEEK=deepseek-v4-pro` (or `deepseek-v4-flash`). If no provider smoke model is
configured, live product smoke fails as `missing_env` unless you explicitly set
`LUICODE_ALLOW_NO_PROVIDER_SMOKE=1`.
...
| `cli` | server entrypoint, Claude CLI adaptive thinking, automatic WebSearch, Auto-mode classifier, session cleanup | Claude CLI binary and provider only for real CLI; connected OpenAI account for Auto mode; `LUICODE_SMOKE_RUN_WEB_TOOLS=1` for WebSearch |
...
| `providers` | multi-turn text, adaptive thinking history, tools, disconnect, errors | configured providers, optional `LUICODE_SMOKE_MODEL_*` |
...
| `nvidia_nim_vision` | Claude-style image tool result reaches a NIM vision model as pixels | `NVIDIA_NIM_API_KEY`, `LUICODE_SMOKE_MODEL_NVIDIA_NIM_VISION` |
...
| `voice` | generated WAV through local Whisper or NVIDIA NIM transcription | `VOICE_NOTE_ENABLED=true`, `LUICODE_SMOKE_RUN_VOICE=1` |

## Examples

```powershell
$env:LUICODE_LIVE_SMOKE = "1"
$env:LUICODE_SMOKE_PROVIDER_MATRIX = "open_router,nvidia_nim,deepseek,lmstudio,llamacpp,ollama"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:LUICODE_LIVE_SMOKE = "1"
$env:LUICODE_SMOKE_TARGETS = "ollama"
...
```powershell
$env:LUICODE_LIVE_SMOKE = "1"
$env:LUICODE_SMOKE_TARGETS = "telegram,discord,voice"
$env:LUICODE_SMOKE_RUN_VOICE = "1"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:LUICODE_LIVE_SMOKE = "1"
$env:LUICODE_SMOKE_TARGETS = "nvidia_nim_cli"
$env:LUICODE_SMOKE_NIM_MODELS = "nvidia/nemotron-3.5-lightning-30b-a3b,moonshotai/kimi-k3,minimaxai/minimax-m3,nvidia/nemotron-3-super-120b-a12b"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:LUICODE_LIVE_SMOKE = "1"
$env:LUICODE_SMOKE_TARGETS = "openrouter_free_cli"
$env:LUICODE_SMOKE_OPENROUTER_FREE_MODELS = "nvidia/nemotron-3-super-120b-a12b:free,poolside/laguna-s-2.1:free,poolside/laguna-xs-2.1:free"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:LUICODE_LIVE_SMOKE = "1"
$env:LUICODE_SMOKE_TARGETS = "cli"
$env:LUICODE_SMOKE_PROVIDER_MATRIX = "openai"
$env:LUICODE_SMOKE_MODEL_OPENAI = "gpt-5.6-luna"
...
```powershell
$env:LUICODE_LIVE_SMOKE = "1"
$env:LUICODE_SMOKE_TARGETS = "nvidia_nim_vision"
$env:LUICODE_SMOKE_MODEL_NVIDIA_NIM_VISION = "meta/llama-3.2-11b-vision-instruct"
...
```bash
LUICODE_LIVE_SMOKE=1 \
LUICODE_SMOKE_TARGETS=nvidia_nim_vision \
LUICODE_SMOKE_MODEL_NVIDIA_NIM_VISION=meta/llama-3.2-11b-vision-instruct \
uv run pytest smoke/product/test_nvidia_nim_vision_product_live.py -n 0 -s --tb=short
```
...
```powershell
$env:LUICODE_LIVE_SMOKE = "1"
$env:LUICODE_SMOKE_TARGETS = "clients"
uv run pytest smoke/product/test_codex_modes_product_live.py -n 0 -s -k local_e2e
```

For a real model smoke, set `LUICODE_SMOKE_CODEX_FREE_MODEL` to an explicitly chosen
...
- Runtime settings use the isolated managed `~/.luicode/.env`; `LUICODE_ENV_FILE` is
  exercised only by the one-time legacy migration smoke.
- `LUICODE_LIVE_SMOKE=1`: enables live smoke execution.
- `LUICODE_ALLOW_NO_PROVIDER_SMOKE=1`: permits no-provider live smoke for harness work.
- `LUICODE_SMOKE_TARGETS`: comma-separated targets, or `all`.
- `LUICODE_SMOKE_PROVIDER_MATRIX`: comma-separated provider prefixes to require.
- `LUICODE_SMOKE_MODEL_<PROVIDER>`: optional per-provider smoke model override.
  Use the uppercase provider ID, such as `LUICODE_SMOKE_MODEL_KILO`. Values may
  include the provider prefix or just the model name for that provider.
- `LUICODE_SMOKE_MODEL_NVIDIA_NIM_VISION`: required explicit NIM vision model for
  the opt-in `nvidia_nim_vision` target; it never falls back to the text model.
- `LUICODE_SMOKE_MODEL_MISTRAL_REASONING`: optional override for the dedicated
  Mistral native reasoning smoke, default `mistral/mistral-medium-3-5`.
- `LUICODE_SMOKE_NIM_MODELS`: optional comma-separated NVIDIA NIM CLI matrix models
  that replace the default characterization set.
- `LUICODE_SMOKE_NIM_EXTRA_MODELS`: optional comma-separated NVIDIA NIM CLI matrix
  models appended to the default or replacement set.
- `LUICODE_SMOKE_OPENROUTER_FREE_MODELS`: optional comma-separated OpenRouter free
  CLI matrix models that replace the default characterization set.
- `LUICODE_SMOKE_OPENROUTER_FREE_EXTRA_MODELS`: optional comma-separated OpenRouter
  free CLI matrix models appended to the default or replacement set.
- `LUICODE_SMOKE_TIMEOUT_S`: per-request/subprocess timeout, default `45`.
- `LUICODE_SMOKE_CLAUDE_BIN`: Claude CLI executable name, default `claude`.
- `LUICODE_SMOKE_RUN_WEB_TOOLS=1`: enables the combined real-provider automatic
  WebSearch and installed Claude Code WebSearch scenario, including a public
  DuckDuckGo request.
- `LUICODE_SMOKE_TELEGRAM_CHAT_ID`: Telegram chat/user ID for send/edit/delete.
- `LUICODE_SMOKE_DISCORD_CHANNEL_ID`: Discord channel ID for send/edit/delete.
- `LUICODE_SMOKE_INTERACTIVE=1`: enables manual inbound Telegram/Discord checks.
- `LUICODE_SMOKE_RUN_VOICE=1`: allows voice transcription backends to load/run.
...
`uv run`, so Windows does not try to replace `luicode-server.exe` while it is
locked.
...
- `target_disabled`: skipped because `LUICODE_SMOKE_TARGETS` intentionally selected
  a different target.
...
explicitly selected a provider in `LUICODE_SMOKE_PROVIDER_MATRIX`;
