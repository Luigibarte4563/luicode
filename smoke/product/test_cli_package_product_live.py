...
from smoke.lib.child_process import cmd_luicode_version, run_captured_text
...
def test_entrypoint_version_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    ...
    assert result.stdout == f"luicode {package_version()}\n"
    assert result.stderr == ""
    assert not (tmp_path / ".luicode" / ".env").exists()


@pytest.mark.asyncio
async def test_cli_session_resume_fork_e2e(tmp_path: Path) -> None:
    session = ManagedClaudeSession(
        str(tmp_path), "http://127.0.0.1:8082", auth_token="luicode"
    )
    process = AsyncMock()
...
async def test_cli_session_stop_kills_child_e2e(tmp_path: Path) -> None:
    ...
    with patch("luicode.cli.managed.session.kill_pid_tree_best_effort") as kill_tree:
