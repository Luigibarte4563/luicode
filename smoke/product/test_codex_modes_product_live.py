"""Installed Codex modes through LUICODE; local cases never use provider credentials."""
...
def _environment(tmp_path: Path, model: str) -> tuple[dict[str, str], set[str]]:
    ...
    atomic_write_managed_config(
        ...
        path=tmp_path / "home" / ".luicode" / ".env",
    )
    ...
    unset.update({"LUICODE_ENV_FILE", "OPENAI_API_KEY", "CODEX_API_KEY"})
    return env, unset
        ...
        def do_POST(self) -> None:
            ...
            try:
                ...
                child_request = child_finished is not None and not any(
                    "LUICODE_PARENT_REVIEW_WORK"
                    in json.dumps(message.get("content", ""))
                ...
                if "outcome" in schema.get("properties", {}):
                    ...
                    arguments = {
                        "message": "LUICODE_CHILD_REVIEW_WORK: Execute the disposable local smoke command, then finish."
                    }
                    ...
                    if child_request and child_finished is not None:
                        child_finished.set()
                    delta = {"content": "LUICODE_MODE_SMOKE_DONE"}
                    reason = "stop"
                self.send_response(200)
...
def test_codex_modes_local_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    ...
    with _canned_provider() as (url, state):
        ...
        try:
            ...
            # A new LUICODE process must restore the original native settings when
        ...
        async with client.stream(
            ...
            try:
                async with asyncio.timeout(timeout_s):
                    await queue.get()
                    first = await send(
                        "LUICODE_PARENT_REVIEW_WORK: Delegate a harmless local command to one child agent."
                    )
                    review = None
                    parent_done = False
    ...
    with _canned_provider(release, finished) as (url, state):
        ...
        try:
            with SmokeServerDriver(
                ...
                with closing(
                    sqlite3.connect(tmp_path / "home" / ".luicode" / "code" / "code.db")
                ) as database:
...
def test_codex_modes_free_provider_e2e(
    ...
    model = os.getenv("LUICODE_SMOKE_CODEX_FREE_MODEL")
    if not model:
        pytest.skip(
            "missing_env: select LUICODE_SMOKE_CODEX_FREE_MODEL to run free live inference"
        )
