...
from luicode.config.paths import managed_env_path
...
def test_pi_cli_prompt_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    ...
    with SmokeServerDriver(
        ...
        result = subprocess.run(
            [
                ...
                "--no-approve",
                "--print",
                "Reply with exactly LUICODE_SMOKE_PI",
            ],
            cwd=tmp_path,
            env=env,
            check=False,
    ...
    assert "LUICODE_SMOKE_PI" in result.stdout
...
def test_opencode_cli_prompt_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    ...
    for path in (isolated_home, isolated_config):
        path.mkdir()
    isolated_luicode = isolated_home / ".luicode"
    isolated_luicode.mkdir(mode=0o700)
    shutil.copyfile(managed_env_path(), isolated_luicode / ".env")
    isolated_env = {
    ...
    with SmokeServerDriver(
        ...
        command = cmd_python_c(
            "from luicode.cli.launchers.opencode import launch; launch()"
        )
        ...
        marker = f"LUICODE_SMOKE_OPENCODE_{uuid.uuid4().hex}"
        (tmp_path / "luicode-smoke-marker.txt").write_text(marker, encoding="utf-8")
        output = run(
            "run",
            "--format",
            "json",
            "--model",
            f"luicode/{provider_model.full_model}",
            "--auto",
            "Use the file-reading tool to read luicode-smoke-marker.txt. "
    ...
    assert not list((isolated_luicode / "tmp" / "launchers").iterdir())
...
def test_aider_cli_prompt_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    ...
    with SmokeServerDriver(
        ...
        result = subprocess.run(
            [
                ...
                "--message",
                "Reply with exactly LUICODE_SMOKE_AIDER",
            ],
            cwd=tmp_path,
            env=env,
            check=False,
    ...
    assert result.returncode == 0, result.stderr or result.stdout
    assert "LUICODE_SMOKE_AIDER" in result.stdout
    assert "POST /v1/messages" in server_log
    assert "POST /v1/responses" not in server_log
    assert not any((isolated_home / ".luicode" / "tmp" / "aider").iterdir())
...
def test_cline_cli_prompt_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    ...
    with SmokeServerDriver(
        ...
        result = subprocess.run(
            [
                ...
                "--model",
                provider_model.full_model,
                "Reply with exactly LUICODE_SMOKE_CLINE",
            ],
            cwd=tmp_path,
            env=env,
            check=False,
    ...
    assert result.returncode == 0, result.stderr or result.stdout
    assert "LUICODE_SMOKE_CLINE" in result.stdout
    assert "POST /v1/responses" in server_log
    assert "POST /v1/chat/completions" not in server_log
...
def test_hermes_cli_prompt_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    ...
    model_id = "luicode-smoke-hermes"
    full_model = f"lmstudio/{model_id}"
    marker = "LUICODE_SMOKE_HERMES"
    auth_token = smoke_config.settings.proxy_auth_token
    ...
    with (
        ...
        for key in tuple(env):
            if key.startswith("LUICODE_HERMES_"):
                env.pop(key)
        env.update(
            {
                "HOST": "127.0.0.1",
...
def test_dsh_cli_headless_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    ...
    model_id = "luicode-smoke-dsh"
    full_model = f"lmstudio/{model_id}"
    marker = "LUICODE_SMOKE_DSH"
    auth_token = smoke_config.settings.proxy_auth_token
...
def test_dsh_cli_terminal_failure_e2e(
    ...
    full_model = "lmstudio/luicode-smoke-failing-model"
    auth_token = smoke_config.settings.proxy_auth_token
    credential_env_keys = _provider_credential_env_keys()
    with (
        ...
        result = subprocess.run(
            [
                ...
                "--profile",
                "headless",
                "Reply with exactly LUICODE_SMOKE_UNREACHABLE",
            ],
            cwd=tmp_path,
            env=env,
            check=False,
...
def test_dsh_cli_web_startup_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    ...
    model_id = "luicode-smoke-dsh-web"
    full_model = f"lmstudio/{model_id}"
    auth_token = smoke_config.settings.proxy_auth_token
    credential_env_keys = _provider_credential_env_keys()
...
def test_grok_cli_headless_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    ...
    model_id = "luicode-smoke-grok"
    full_model = f"lmstudio/{model_id}"
    marker = "LUICODE_SMOKE_GROK"
    auth_token = smoke_config.settings.proxy_auth_token
    credential_env_keys = _provider_credential_env_keys()
    ...
    with (
        ...
        for key in credential_env_keys:
            env.pop(key, None)
        for key in tuple(env):
            if key.startswith("LUICODE_GROK_") or key.startswith("GROK_"):
                env.pop(key)
...
def test_muse_cli_headless_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    ...
    model_id = "luicode-smoke-muse"
    full_model = f"lmstudio/{model_id}"
    marker = "LUICODE_SMOKE_MUSE"
    auth_token = smoke_config.settings.proxy_auth_token
    credential_env_keys = _provider_credential_env_keys()
    ...
    with (
        ...
        for key in credential_env_keys:
            env.pop(key, None)
        for key in tuple(env):
            if key.startswith("LUICODE_MUSE_"):
                env.pop(key)
        for key in (
            "META_API_KEY",
            "MUSE_MODEL",
    ...
    assert full_model in visible_models, "Muse hides the configured LUICODE model"
    responses_count = server_log.count("POST /v1/responses")
    assert responses_count >= 1
    assert responses_count == len(provider_requests)
    assert "POST /v1/chat/completions" not in server_log
...
def test_claude_cli_adaptive_thinking_e2e(
    ...
    with SmokeServerDriver(
        ...
        result = ClientProtocolDriver.run_claude_prompt(
            ...
            config=smoke_config,
            cwd=tmp_path,
            prompt="think hard, then reply with exactly LUICODE_SMOKE_CLI",
        )
        server_log = server.log_path.read_text(encoding="utf-8", errors="replace")
    ...
    assert "LUICODE_SMOKE_CLI" in result.stdout


@pytest.mark.smoke_target("cli")
def test_claude_cli_web_search_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    if os.environ.get("LUICODE_SMOKE_RUN_WEB_TOOLS") != "1":
        pytest.skip("missing_env: set LUICODE_SMOKE_RUN_WEB_TOOLS=1")
    claude_bin = shutil.which(smoke_config.claude_bin)
    if not claude_bin:
        pytest.skip(f"missing_env: Claude CLI not found: {smoke_config.claude_bin}")
    provider_model = ProviderMatrixDriver(smoke_config).first_model()
    ...
    with SmokeServerDriver(
        ...
        run = run_claude_cli(
            ...
            bare=False,
            prompt=(
                "Use WebSearch exactly once to find the luicode GitHub "
                "repository. Then reply with LUICODE_SMOKE_WEB_SEARCH and one source URL."
            ),
            tools="WebSearch",
        )
        server_log = server.log_path.read_text(encoding="utf-8", errors="replace")
    ...
    assert "LUICODE_SMOKE_WEB_SEARCH" in run.combined_output
    ...
    assert (
        sum(
            row.get("event") == "luicode.api.web_search.automatic_recognized"
            for row in log_rows
        )
        == 1
    ), server_log
    assert any(
        row.get("event") == "luicode.api.optimization.web_server_tool"
        for row in log_rows
    ), server_log
    assert (
        sum(
            row.get("event") == "luicode.api.web_search.automatic_selected"
            for row in log_rows
        )
        == 1
    ), server_log
    assert (
        sum(
            row.get("event") == "luicode.api.web_search.automatic_completed"
            for row in log_rows
        )
        == 1
    ), server_log
...
def test_claude_auto_mode_openai_connected_e2e(
    ...
    if provider_model is None:
        pytest.skip(
            "missing_env: set LUICODE_SMOKE_MODEL_OPENAI to run the connected-account "
            "Auto-mode smoke"
        )

    marker = f"LUICODE_SMOKE_AUTO_MODE_{uuid.uuid4().hex}"
    ...
    policy_rows = [
        row
        for row in log_rows
        if row.get("event") == "luicode.api.route.safety_classifier_policy"
    ]
    assert any(
...
def test_claude_cli_provider_error_e2e(
    ...
    if not claude_bin:
        pytest.skip(f"missing_env: Claude CLI not found: {smoke_config.claude_bin}")
    broken_model = "lmstudio/luicode-smoke-failing-model"
    ...
    with (
        ...
        result = ClientProtocolDriver.run_claude_prompt(
            ...
            config=smoke_config,
            cwd=tmp_path,
            prompt="Reply with exactly LUICODE_SMOKE_UNREACHABLE.",
    ...
    assert "luicode smoke provider rejected the request deliberately" in lower
...
def _isolated_dsh_env(
    *,
    ...
    for key in credential_env_keys:
        env.pop(key, None)
    for key in tuple(env):
        if key.startswith("LUICODE_DSH_"):
            env.pop(key)
    env.update(
        {
            "HOST": "127.0.0.1",
            ...
            if self.path == "/v1/models":
                self._write_json(
                    HTTPStatus.OK,
                    {
                        ...
                        "data": [
                            {
                                "id": "luicode-smoke-failing-model",
                                "object": "model",
                            }
                        ],
                    },
            ...
            self._write_json(
                HTTPStatus.BAD_REQUEST,
                {
                    "error": {
                        ...
                        "code": "luicode_smoke_failure",
                        "message": "LUICODE smoke provider rejected the request deliberately.",
                    }
                },
            )
