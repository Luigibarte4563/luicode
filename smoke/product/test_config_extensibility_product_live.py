...
from luicode.providers.runtime import build_provider_config
...
def _write_managed_config(home: Path, text: str) -> None:
    path = home / ".luicode" / ".env"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(f"LUICODE_CONFIG_SCHEMA=1\n{text}", encoding="utf-8")
...
def test_removed_env_migration_e2e(smoke_config: SmokeConfig, tmp_path) -> None:
    ...
    env = _isolated_config_env(home)
    env["LUICODE_ENV_FILE"] = str(env_file)
    script = (
        "from luicode.config.loader import get_settings; print(get_settings().model)"
    )
    result = run_captured_text(
        cmd_python_c(script),
        cwd=smoke_config.root,
