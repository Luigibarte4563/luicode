...
(``luicode-server.exe`` in use), causing flaky smoke. The smoke runner is
...
def cmd_luicode_version() -> list[str]:
    return [
        python_exe(),
        "-c",
        (
            "import sys; "
            "sys.argv = ['luicode-server', '--version']; "
            "from luicode.cli.entrypoints import serve; serve()"
        ),
    ]


def cmd_luicode_server() -> list[str]:
    return [
        python_exe(),
        "-c",
        "from luicode.cli.entrypoints import serve; serve()",
