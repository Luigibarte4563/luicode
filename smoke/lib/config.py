...
from luicode.config.loader import clear_settings_cache, get_settings
from luicode.config.model_refs import parse_model_name, parse_provider_type
...
from luicode.providers.runtime.config import has_provider_configuration
...
TARGET_REQUIRED_ENV: dict[str, tuple[str, ...]] = {
    "api": (),
    "auth": (),
    "cli": ("LUICODE_SMOKE_CLAUDE_BIN", "configured provider for Claude CLI prompt"),
    "clients": (),
    "config": (),
    "extensibility": (),
    "messaging": (),
    "providers": (
        "configured provider credentials/endpoints or LUICODE_SMOKE_MODEL_*",
    ),
    ...
    "nvidia_nim_cli": (
        "NVIDIA_NIM_API_KEY",
        "LUICODE_SMOKE_CLAUDE_BIN or claude on PATH",
    ),
    "nvidia_nim_vision": (
        "NVIDIA_NIM_API_KEY",
    ...
    "telegram": (
        "TELEGRAM_BOT_TOKEN",
        "ALLOWED_TELEGRAM_USER_ID or LUICODE_SMOKE_TELEGRAM_CHAT_ID",
    ),
    "discord": (
        "DISCORD_BOT_TOKEN",
        "ALLOWED_DISCORD_CHANNELS or LUICODE_SMOKE_DISCORD_CHANNEL_ID",
    ),
    "voice": ("VOICE_NOTE_ENABLED=true", "LUICODE_SMOKE_RUN_VOICE=1"),
}
...
class SmokeConfig:
    ...
    def load(cls) -> SmokeConfig:
        ...
        return cls(
            ...
            live=os.getenv("LUICODE_LIVE_SMOKE") == "1",
            interactive=os.getenv("LUICODE_SMOKE_INTERACTIVE") == "1",
            targets=_parse_targets(os.getenv("LUICODE_SMOKE_TARGETS")),
            provider_matrix=_parse_csv(os.getenv("LUICODE_SMOKE_PROVIDER_MATRIX")),
            timeout_s=float(os.getenv("LUICODE_SMOKE_TIMEOUT_S", "45")),
            prompt=os.getenv(
                "LUICODE_SMOKE_PROMPT", "Reply with exactly: LUICODE_SMOKE_PONG"
            ),
            claude_bin=os.getenv("LUICODE_SMOKE_CLAUDE_BIN", "claude"),
    ...
    def nvidia_nim_vision_model(self) -> ProviderModel | None:
        ...
        override_env = "LUICODE_SMOKE_MODEL_NVIDIA_NIM_VISION"
    ...
    def mistral_reasoning_smoke_model(self) -> ProviderModel | None:
        ...
        if not self.has_provider_configuration("mistral"):
            return None
        override_env = "LUICODE_SMOKE_MODEL_MISTRAL_REASONING"
    ...
    def _include_provider_in_smoke(
        ...
        if self.provider_matrix and provider in self.provider_matrix:
            return True
        return bool(os.getenv(f"LUICODE_SMOKE_MODEL_{provider.upper()}"))
...
def _provider_smoke_model(provider: str) -> tuple[str, str]:
    override_env = f"LUICODE_SMOKE_MODEL_{provider.upper()}"
    if override := os.getenv(override_env):
...
def _normalize_provider_model(provider: str, raw_model: str) -> str:
    ...
    if not model:
        msg = f"LUICODE_SMOKE_MODEL_{provider.upper()} must not be empty"
...
def nvidia_nim_cli_model_refs(
    ...
    explicit_models = _parse_csv_ordered(source.get("LUICODE_SMOKE_NIM_MODELS"))
    extra_models = _parse_csv_ordered(source.get("LUICODE_SMOKE_NIM_EXTRA_MODELS"))

    if "LUICODE_SMOKE_NIM_MODELS" in source and not explicit_models:
        raise ValueError("LUICODE_SMOKE_NIM_MODELS must list at least one model")

    models: list[tuple[str, str]] = []
    base_models = explicit_models or NVIDIA_NIM_CLI_DEFAULT_MODELS
    base_source = (
        "LUICODE_SMOKE_NIM_MODELS" if explicit_models else "nvidia_nim_cli_default"
    )
    models.extend((model, base_source) for model in base_models)
    models.extend((model, "LUICODE_SMOKE_NIM_EXTRA_MODELS") for model in extra_models)
...
def openrouter_free_cli_model_refs(
    ...
    explicit_models = _parse_csv_ordered(
        source.get("LUICODE_SMOKE_OPENROUTER_FREE_MODELS")
    )
    extra_models = _parse_csv_ordered(
        source.get("LUICODE_SMOKE_OPENROUTER_FREE_EXTRA_MODELS")
    )

    if "LUICODE_SMOKE_OPENROUTER_FREE_MODELS" in source and not explicit_models:
        raise ValueError(
            "LUICODE_SMOKE_OPENROUTER_FREE_MODELS must list at least one model"
        )
    ...
    models.extend((model, base_source) for model in base_models)
    models.extend(
        (model, "LUICODE_SMOKE_OPENROUTER_FREE_EXTRA_MODELS") for model in extra_models
    )
