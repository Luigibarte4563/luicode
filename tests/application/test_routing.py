...
from luicode.application.errors import UnknownProviderError
...
from luicode.core.reasoning import ReasoningControl, ReasoningEffort


@pytest.mark.parametrize(
    "prefix,off",
    [("", False), ("anthropic/", False), ("claude-3-luicode-no-thinking/", True)],
)
@pytest.mark.parametrize(
...
def test_model_router_routes_no_thinking_gateway_model_directly(settings):
    routed = ModelRouter(settings).resolve_messages_request(
        MessagesRequest(
            model="claude-3-luicode-no-thinking/nvidia_nim/deepseek-ai/deepseek-v4-pro",
            max_tokens=100,
            messages=[Message(role="user", content="hello")],
        )
    )
    ...
    assert (
        routed.resolved.original_model
        == "claude-3-luicode-no-thinking/nvidia_nim/deepseek-ai/deepseek-v4-pro"
    )
    assert routed.resolved.primary.provider_id == "nvidia_nim"
    assert routed.resolved.primary.provider_model == "deepseek-ai/deepseek-v4-pro"
...
def test_model_router_logs_mapping(settings):
    with patch("luicode.application.routing.logger.debug") as mock_log:
