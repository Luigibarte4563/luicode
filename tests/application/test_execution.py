...
from luicode.core.anthropic.models import Message, MessagesRequest
from luicode.core.async_iterators import AsyncCloseable
...
from luicode.core.reasoning import ReasoningCapability, ReasoningPolicy
...
async def test_retryable_preframe_failure_selects_fallback_after_closing_primary() -> (
    ...
    with patch("luicode.application.execution.trace_event") as trace_mock:
        chunks = [
            chunk
    ...
    started = next(
        event
        for event in events
        if event.get("event") == "luicode.model_fallback.started"
    )
    assert started == {
        "stage": "execution",
        "event": "luicode.model_fallback.started",
        "source": "application",
        "request_id": "req_fallback",
        "wire_api": "messages",
    ...
    selected = next(
        event
        for event in events
        if event.get("event") == "luicode.model_fallback.selected"
    )
...
async def test_nonretryable_provider_failure_selects_fallback_before_first_frame(
    ...
    fallback_started = next(
        ...
        if call.kwargs.get("event") == "luicode.model_fallback.started"
    )
...
async def test_progress_timeout_before_first_chunk_is_canonical_and_correlated() -> (
    ...
    timeout_traces = [
        call.kwargs
        for call in trace_mock.call_args_list
        if call.kwargs.get("event") == "luicode.provider.progress_timeout"
    ]
    assert timeout_traces == [
        {
            "stage": "execution",
            "event": "luicode.provider.progress_timeout",
            "source": "application",
            "request_id": request_id,
...
async def test_provider_cleanup_cannot_delay_fallback_past_progress_deadline() -> None:
    ...
    with (
        patch("luicode.application.execution.trace_event") as trace_mock,
    ...
    assert fallback.stream_calls == []
    assert all(
        call.kwargs.get("event") != "luicode.model_fallback.started"
        for call in trace_mock.call_args_list
    )
...
async def test_cleanup_failure_trace_names_preserved_provider_failure() -> None:
    ...
    with (
        patch("luicode.core.trace.trace_event") as trace_mock,
        pytest.raises(ExecutionFailure) as exc_info,
    ):
        await anext(stream)
...
async def test_fallback_transition_does_not_reset_shared_progress_deadline() -> None:
    ...
    with (
        patch("luicode.application.execution.monotonic", side_effect=[0, 1, 2, 4]),
        patch("luicode.application.execution.trace_event") as trace_mock,
        patch(
            "luicode.application.execution.asyncio.timeout_at",
