...
from luicode.application.ports import ModelCatalogSnapshot
...
def test_catalog_sorts_components_and_preserves_exact_identity_and_default():
    ...
    assert catalog.default_model_id == "claude-3-luicode-no-thinking/open_router/Zulu"
    selected = next(
...
def test_catalog_wire_slug_prefers_the_advertised_no_thinking_slug() -> None:
    models = (
        _client_model(
            "claude-3-luicode-no-thinking/open_router/vendor/chat-model",
