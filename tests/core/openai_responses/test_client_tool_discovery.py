...
from luicode.core.anthropic.stream_contracts import parse_sse_text
from luicode.core.json_types import JsonObject, JsonValue
from luicode.core.openai_responses import (
...
from luicode.core.openai_tool_names import encode_openai_chat_tool_names
from luicode.providers.openai_chat.stream_output import (
    ChatStreamUsage,
    ResponsesChatStreamOutput,
)
from luicode.providers.openai_responses.presentation import (
...
def test_interrupted_discovery_keeps_completed_occurrences_of_reused_id(
    ...
    assert set(names) == {"first", "last", "luicode_tool_search"}
    assert request.model_dump() == original
...
def test_integral_json_numbers_are_accepted_by_native_codex(search: bool) -> None:
    ...
    value = adapter.restore_item(
        {
            "type": "function_call",
            "name": "luicode_tool_search" if search else "agents__spawn_agent",
            "status": "completed",
...
def test_namespaced_search_named_tool_keeps_its_identity_in_all_events(
    ...
    events = _completed_tool_events(
        request,
        native=native,
        name="ordinary__luicode_tool_search",
    ...
    for event in events:
        if "item" in event:
            ...
            assert event["item"]["name"] == "luicode_tool_search"
...
def test_discovery_preserves_outer_namespace_in_nested_tools(
    ...
    if native:
        ...
        assert {
            tool["function"]["name"]
            for tool in cast(list[dict[str, Any]], body["tools"])
        } == {"luicode_tool_search", "agents__run"}
        assert body["tool_choice"] == {
            "type": "function",
    ...
    request = OpenAIResponsesRequest(
        ...
        tools=[
            SEARCH,
            {
                ...
                "name": "editor",
                "tools": [
                    {"type": "custom", "name": "luicode_tool_search"},
                ],
            },
            {
                "type": "namespace",
...
def test_interleaved_tool_kinds_keep_arguments_and_terminal_output_consistent(
    ...
    names = ["luicode_tool_search", "group__lookup", "group__edit"]
...
def test_argument_numbers_survive_sse_serialization(
    ...
    name = "luicode_tool_search" if search else "lookup"
    if native:
        adapter = ResponsesToolAdapter(
            request,
            ResponsesToolPolicy(
...
def test_client_search_reserves_hosted_discovery_names_before_replay(
    ...
    assert adapter.request.tools[0]["name"] == "luicode_tool_search_2"
    call = cast(
        dict[str, Any],
        adapter.restore_item(
            {
    ...
    assert call["name"] == "luicode_tool_search"
    search = cast(
        dict[str, Any],
        adapter.restore_item(
            {
    ...
    assert (
        replay.request.tools
        and replay.request.tools[0]["name"] == "luicode_tool_search_2"
    )
    assert (
