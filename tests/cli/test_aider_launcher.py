"""Native Aider configuration replaces LUICODE argument rewriting."""
...
def test_aider_receives_native_files_and_a_private_credential_reference(
    ...
    monkeypatch.setenv("luicode_aider_proxy_auth_stale", "stale-secret")
    ...
    def inspect(command, env):
        ...
        assert "luicode_aider_proxy_auth_stale" not in env
        (key_name,) = [
            key for key in env if key.startswith("LUICODE_AIDER_PROXY_AUTH_")
        ]
        ...
        launch_id = bare["extra_params"]["extra_headers"]["x-luicode-launch-id"]
        assert launch_id
        launch_ids.append(launch_id)
        assert all(
            entry["extra_params"]["extra_headers"] == {"x-luicode-launch-id": launch_id}
        ...
        assert bare["extra_params"] == {
            ...
            "extra_headers": {"x-luicode-launch-id": launch_id},
        }
        ...
        assert command[1:3] == ["--set-env", "ANTHROPIC_API_KEY=luicode-local"]
