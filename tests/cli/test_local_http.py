"""Local LUICODE transport must never escape through an outbound proxy."""
...
from luicode.cli.launchers.common import preflight_proxy
...
def test_proxy_preflight_connects_directly_when_http_proxy_is_configured(
    ...
    with _status_server(502) as (forward_proxy_url, forward_proxy_hits):
        ...
        with _status_server(200) as (luicode_url, luicode_hits):
            assert preflight_proxy(luicode_url) is None

    assert luicode_hits == ["/health"]
