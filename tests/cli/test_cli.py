...
class TestManagedClaudeSession:
    ...
    def test_session_extract_session_id(self):
        ...
        from luicode.cli.managed.claude import (
    ...
    async def test_start_task_basic_flow(self):
        ...
        session = ManagedClaudeSession(
            "/tmp", "http://localhost:8082", auth_token="luicode"
        )
    ...
    async def test_drain_stderr_bounded_retains_cap_but_drains_to_eof(self):
        ...
        from luicode.cli.managed.session import (
    ...
    async def test_stop_session(self):
        ...
        with patch(
            "luicode.cli.managed.session.kill_pid_tree_best_effort"
        ) as kill_tree:
    ...
    async def test_start_task_uses_sentinel_when_proxy_auth_blank(self):
        ...
        with (
            ...
            env = mock_exec.call_args.kwargs["env"]
            assert env["ANTHROPIC_AUTH_TOKEN"] == "luicode"
    ...
    async def test_start_task_json_error(self):
        """Test handling of non-JSON output from luicode.cli."""
        from luicode.cli.managed.session import ManagedClaudeSession
