...
from luicode.cli.launchers import pi
...
def test_pi_registers_bundled_extension_without_a_python_catalog(
    ...
    assert command[3:5] == ["--models", "luicode/**"]
    assert launch_capture.environments[0]["LUICODE_PI_API_KEY"] == "launcher-test-token"
    assert (
        launch_capture.environments[0]["LUICODE_PI_BASE_URL"] == "http://127.0.0.1:8182"
    )
