...
from luicode.harnesses import resources
...
def test_cleanup_on_process_start_failure(
    ...
    if isinstance(failure, KeyboardInterrupt):
        with pytest.raises(KeyboardInterrupt):
            importlib.import_module(f"luicode.cli.launchers.{name}").launch([])
    else:
...
def test_interrupt_kills_and_waits_for_registered_child_before_cleanup(
    ...
    with (
        ...
        importlib.import_module("luicode.cli.launchers.codex").launch([])
