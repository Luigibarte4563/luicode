"""Contracts for Aider's process-local LUICODE model files."""
...
from luicode.cli.launchers.aider_config import build_aider_config
...
def test_aider_config_projects_messages_route_and_canonical_catalog() -> None:
    config = build_aider_config(
        ...
        api_key_env="LUICODE_AIDER_PROXY_AUTH_A1B2C3",
        launch_id="launch-a",
    )
    ...
    for wire_name, metadata in expected_metadata.items():
        for name in (wire_name, f"anthropic/{wire_name}"):
            ...
            assert entry["extra_params"] == {
                ...
                "api_key": "os.environ/LUICODE_AIDER_PROXY_AUTH_A1B2C3",
                "extra_headers": {"x-luicode-launch-id": "launch-a"},
            }
...
def test_aider_catalog_ids_take_precedence_over_generated_transport_spellings() -> None:
    ...
    config = build_aider_config(
        ...
        api_key_env="LUICODE_AIDER_PROXY_AUTH_COLLISION",
        launch_id="launch-a",
    )
    entries = {entry["name"]: entry for entry in config.settings}
...
@pytest.mark.parametrize(
    ...
    [
        "",
        "LUICODE_AIDER_PROXY_AUTH_",
        "ANTHROPIC_API_KEY",
        "LUICODE_AIDER_PROXY_AUTH_lowercase",
        "LUICODE_AIDER_PROXY_AUTH_BAD-NAME",
        "LUICODE_AIDER_PROXY_AUTH_BAD/NAME",
    ],
)
