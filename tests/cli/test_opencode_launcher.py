...
from luicode.cli.launchers.opencode_config import build_opencode_config
...
def test_opencode_config_uses_native_responses_and_model_budgets() -> None:
    ...
    luicode = provider["luicode"]
    assert isinstance(luicode, dict)
    assert luicode["package"] == "@opencode/ai/providers/openai/responses"
    assert luicode["settings"] == {
        "baseURL": "http://127.0.0.1:9191/v1",
        "apiKey": "{env:LUICODE_OPENCODE_API_KEY}",
    }
    assert luicode["models"] == {
    ...
    assert config.overlay == {
        ...
        "model": "luicode/nvidia_nim/vendor/model",
        "agents": {"title": {"model": "luicode/nvidia_nim/vendor/model"}},
    }
...
def test_opencode_child_receives_private_catalog_and_overlay(launch_capture) -> None:
    ...
    def inspect(command, env):
        ...
        provider = config["providers"]["luicode"]
        assert provider["settings"]["baseURL"] == "http://127.0.0.1:8182/v1"
        assert "nvidia_nim/catalog-model:variant" in provider["models"]
        assert overlay["experimental"]["policies"][-1]["resource"] == "luicode"
        assert env["LUICODE_OPENCODE_API_KEY"] == "launcher-test-token"
...
def test_opencode_rejects_caller_control_of_server_lifetime(
    ...
    assert not launch_capture.commands
    assert "LUICODE manages OpenCode standalone mode" in capsys.readouterr().err
