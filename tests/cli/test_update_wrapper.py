...
def fixture_wheel(area, url):
    wheel = area / "luicode-1.0.0-py3-none-any.whl"
    dist = "luicode-1.0.0"
...
def test_installed_update_delegates_and_survives_replacement(tmp_path, outcome):
    ...
    env.update(
        ...
        LUICODE_TEST_LAUNCHER=str(
            area
            / "bin"
            / ("luicode-update.cmd" if os.name == "nt" else "luicode-update")
        ),
    )
    if os.name == "nt":
...
[IO.File]::WriteAllText($env:LUICODE_TEST_LAUNCHER, 'exit /b 99')
"""
...
"""
    ...
    with installer_server(body, 503 if outcome == "download-error" else 200) as (
        ...
        launcher = env["LUICODE_TEST_LAUNCHER"]
        command = (
