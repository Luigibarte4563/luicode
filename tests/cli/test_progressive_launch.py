...
from luicode.cli.launchers import common
from luicode.cli.server_socket import ServerSockets
...
def browser_workers(monkeypatch):
    ...
    def track_start(thread):
        if thread.name == "luicode-open-admin-browser":
...
def test_partial_address_failure_closes_every_reserved_socket(monkeypatch):
    from luicode.cli import server_socket
...
def test_existing_server_must_identify_itself_as_luicode(
...
def test_browser_thread_start_failure_does_not_fail_luicode(monkeypatch, reuse):
    settings = Settings()
    browser = MagicMock()
    monkeypatch.setattr(commands.webbrowser, "open", browser)
    monkeypatch.setattr(
...
def _run_browser_shutdown_probe(mode, outcome, directory):
    """Run real LUICODE lifecycle owners with only OS/browser/server dependencies faked."""
    from luicode.cli import desktop, uvicorn_server
    from luicode.runtime import bootstrap
    ...
    if mode in {"server", "desktop"}:
        ...
        with ServerSockets.reserve("127.0.0.1", 0) as owner:
            ...
            try:
                ...
                lock.release()
    patcher.undo()
    print("LUICODE exited while preserving its resource ownership", flush=True)


@pytest.mark.parametrize("mode", ["server", "desktop"])
@pytest.mark.parametrize("outcome", ["automatic", "tray"])
def test_owned_luicode_process_exits_with_stalled_browser(tmp_path, mode, outcome):
...
def _assert_browser_probe_exits(tmp_path, mode, outcome):
    ...
    assert "LUICODE exited while preserving its resource ownership" in completed.stdout
