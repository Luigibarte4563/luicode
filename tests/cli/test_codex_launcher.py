...
from luicode.cli.launchers import codex, runner
...
def test_codex_selects_the_advertised_slug_from_opaque_catalog_data(
    ...
    assert config["model_providers"]["luicode"]["wire_api"] == "responses"
    assert (
        config["model_providers"]["luicode"]["base_url"] == "http://127.0.0.1:8182/v1"
    )
    assert config["model_providers"]["luicode"]["auth"] == {
