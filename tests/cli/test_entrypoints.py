...
def test_cli_scripts_are_registered() -> None:
    ...
    assert pyproject["project"]["scripts"] == {
        ...
        "luicode-pi": "luicode.cli.launchers.pi:launch",
        "luicode-opencode": "luicode.cli.launchers.opencode:launch",
        "luicode-cline": "luicode.cli.launchers.cline:launch",
        "luicode-hermes": "luicode.cli.launchers.hermes:launch",
        "luicode-dsh": "luicode.cli.launchers.dsh:launch",
        "luicode-grok": "luicode.cli.launchers.grok:launch",
        "luicode-muse": "luicode.cli.launchers.muse:launch",
        "luicode-aider": "luicode.cli.launchers.aider:launch",
    }
    assert pyproject["project"]["gui-scripts"] == {
        "luicode-desktop": "luicode.cli.desktop_entrypoint:launch",
    }
...
def test_luicode_server_reports_version_without_side_effects(
    argv: tuple[str, ...],
    capsys: pytest.CaptureFixture[str],
) -> None:
    from luicode.cli import entrypoints

    with patch.object(entrypoints, "package_version", return_value="9.8.7"):
        entrypoints.serve(argv)

    assert capsys.readouterr() == ("luicode 9.8.7\n", "")
...
def test_version_entrypoint_does_not_import_command_runtime() -> None:
    script = "\n".join(
        (
            "import json",
            "import sys",
            "from luicode.cli.entrypoints import serve",
            "serve(['--version'])",
            "forbidden = ('uvicorn', 'fastapi', 'openai', "
            "'luicode.cli.commands', "
            "'luicode.runtime.bootstrap')",
...
def test_non_version_entrypoint_delegates_to_server_command() -> None:
    from luicode.cli import commands, entrypoints
...
def test_explicit_open_admin_waits_for_owned_http_ready():
    from luicode.cli.commands import ServerSupervisor
...
def test_serve_respects_admin_browser_setting(open_admin_browser: bool) -> None:
    from luicode.cli import commands
...
def test_server_startup_repairs_invalid_managed_provider_proxy(
    ...
    from luicode.cli import commands
    from luicode.config.env_files import dotenv_values_from_file
...
def test_serve_supervisor_restarts_when_app_requests_restart() -> None:
    ...
    with (
        ...
        patch("luicode.cli.uvicorn_server.RuntimeServer", side_effect=FakeServer),
        patch(
            "luicode.runtime.bootstrap.build_asgi_app",
...
def test_claude_child_env_targets_current_proxy_config() -> None:
    from luicode.harnesses.claude import build_claude_proxy_env
