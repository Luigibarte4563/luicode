...
from luicode.application.reasoning import client_reasoning_policy
from luicode.cli.launchers.pi import pi_extension_path
...
def test_pi_sends_current_conversation_id_only_on_luicode_requests() -> None:
...
process.env.LUICODE_PI_BASE_URL = "http://luicode.invalid";
process.env.LUICODE_PI_API_KEY = "test-key";
...
for (const [provider, session] of [
    ["luicode", "conversation-a"],
    ["luicode", "conversation-a"],
    ["luicode", "conversation-b"],
    ["luicode", "conversation-a"],
...
def test_pi_extension_projects_known_capabilities_and_preserves_unknown_defaults() -> (
    ...
    payload = {
        ...
        "data": [
            ...
            {
                "id": "claude-3-luicode-no-thinking/provider/text-only",
    ...
    script = """
const { projectLuicodeModels } = await import(process.argv[1]);
const payload = JSON.parse(process.argv[2]);
console.log(JSON.stringify(projectLuicodeModels(payload)));
"""
    ...
    assert [model["id"] for model in projected] == [
        ...
        "claude-3-luicode-no-thinking/provider/text-only",
        "provider/unknown ",
    ]
