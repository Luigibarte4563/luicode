"""Contract tests for the installed `luicode-cline` launcher."""
...
from luicode.cli.launchers.cline_config import (
    CLINE_PROVIDER_ID,
    build_cline_config,
)
        ...
        "providers": {
            "openai-native": {
                "settings": {
                    ...
                    "apiKey": "proxy-token",
                    "headers": {"x-luicode-launch-id": "launch-a"},
...
def test_cline_native_configuration_is_available_to_child(launch_capture) -> None:
    ...
    def inspect(command, env):
        ...
        launch_ids.append(settings["headers"]["x-luicode-launch-id"])
        assert launch_ids[-1]
