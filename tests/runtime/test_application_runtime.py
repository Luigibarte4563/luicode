...
import luicode.messaging.session.persistence as persistence_module
from luicode.application.connected_accounts import (
    ConnectedAccountState,
    ConnectedAccountStatus,
)
from luicode.application.errors import ApplicationUnavailableError
from luicode.application.model_metadata import ProviderModelInfo
from luicode.config.admin.persistence import PreparedAdminUpdate
...
from luicode.messaging.command_context import StopOutcome
from luicode.messaging.platforms.ports import (
    InboundMessageHandler,
    MessagingPlatformComponents,
    MessagingStartupNotice,
)
from luicode.messaging.session import SessionStore
from luicode.messaging.workflow import MessagingWorkflow
...
async def test_public_start_retries_transient_partial_messaging_cleanup() -> None:
    ...
    with (
        ...
        patch(
            "luicode.runtime.application.messaging_platform_factory.create_messaging_components",
...
async def test_composition_records_runtime_before_workspace_setup() -> None:
    ...
    with (
        patch(
            "luicode.runtime.application.os.makedirs",
...
async def test_composition_publishes_startup_notice_after_runtime_and_repair() -> None:
    ...
    with (
        patch(
            "luicode.cli.managed.ManagedClaudeSessionManager",
            return_value=cli_manager,
        ) as manager_constructor,
        patch("luicode.messaging.session.SessionStore"),
        patch(
            "luicode.messaging.workflow.MessagingWorkflow",
...
async def test_folder_picker_is_stopped_before_http_shutdown_drains(monkeypatch):
    ...
    from luicode.cli.uvicorn_server import RuntimeServer
