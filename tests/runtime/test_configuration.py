...
from luicode.config.admin.persistence import prepare_admin_update
from luicode.config.env_files import LUICODE_CONFIG_SCHEMA_ENV
from luicode.config.env_migrations import CONFIG_SCHEMA_VERSION
from luicode.config.loader import ManagedConfigStore
from luicode.config.paths import config_lock_path
...
def test_reads_are_fresh_read_only_snapshots():
    ...
    for port in (8123, 8124):
        ...
        with patch(
            "luicode.config.loader.consolidate_managed_config",
...
def test_reads_and_commits_fail_closed_on_uninitialized_or_newer_schema(schema):
    store = ManagedConfigStore()
    store.initialize()
    text = f"LUICODE_CONFIG_SCHEMA={schema}\nANTHROPIC_AUTH_TOKEN=preserve\n"
    store.path.write_text(text)
    with pytest.raises(ValueError, match="schema"):
        store.read()
    with pytest.raises(ValueError, match="schema"):
        store.commit({"LUICODE_CONFIG_SCHEMA": "1"})
...
def test_cooperative_commits_are_serialized_and_leave_complete_files():
    ...
    def write(port):
        ManagedConfigStore().commit({"LUICODE_CONFIG_SCHEMA": "1", "PORT": str(port)})

    with (
        patch("luicode.config.loader.atomic_write_managed_config", checked_write),
...
def test_atomic_commit_waits_for_an_open_snapshot_reader(monkeypatch):
    store = ManagedConfigStore()
    store.initialize()
    store.commit({"LUICODE_CONFIG_SCHEMA": "1", "PORT": "8123"})
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        try:
            ...
            writer = executor.submit(
                store.commit, {"LUICODE_CONFIG_SCHEMA": "1", "PORT": "8124"}
            )
