...
from luicode.config import paths
...
from luicode.runtime.retired_chat import remove_retired_chat_history
...
async def test_redirected_directory_is_left_untouched(tmp_path, cleanup_logs):
    ...
    database.write_bytes(b"not owned by LUICODE")
    try:
        (root / "chat").symlink_to(outside, target_is_directory=True)
    except OSError as exc:
        pytest.skip(f"Directory symlinks unavailable: {exc}")
    await _start_and_close()
    assert database.read_bytes() == b"not owned by LUICODE"
