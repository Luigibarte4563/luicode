...
from luicode.runtime.codex_catalog import CodexModelCatalogPublisher
...
async def test_recovery_wait_uses_remaining_budget_without_orphaning_waiter(
    ...
    monkeypatch.setattr(
        "luicode.runtime.provider_manager.InitializationWait", lambda: wait
    )
    ...
    try:
        ...
        if cancel:
            ...
            with pytest.raises(ApplicationUnavailableError, match="still starting"):
                await waiting
        monkeypatch.setattr(
            "luicode.runtime.provider_manager.InitializationWait",
            InitializationWait,
        )
        release.set()
