...
from luicode.application.ports import ModelCatalogSnapshot, RequestRuntimePort
from luicode.config.paths import launcher_temp_dir_path
from luicode.config.settings import Settings
from luicode.core.gateway_model_ids import no_thinking_gateway_model_id
from luicode.runtime.code_sessions_sqlite import SQLiteCodeStore
from luicode.runtime.codex_app_server import (
    CodexAppServer,
    CodexHarnessFactory,
)
...
async def test_factory_open_preserves_setup_and_catalog_until_process_exit(
    ...
    try:
        ...
        assert config["model_providers"]["luicode"] == {
            "name": "luicode",
...
@pytest.mark.asyncio
async def test_history_open_needs_no_luicode_setup_or_inventory(tmp_path, factory_peer):
    runtime = MagicMock(spec=RequestRuntimePort)
    for name in (
        "current_settings",
        "cached_model_info",
...
async def test_complete_mode_overrides_restore_native_defaults(
    ...
    assert rpc.call_args.args[1] == {"cwd": str(tmp_path), "modelProvider": "luicode"}
