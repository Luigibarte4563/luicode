...
from luicode.core.reasoning import DEFAULT_REASONING_POLICY
from luicode.providers.admission import (
    ProviderAdmissionController,
    ProviderOperationKind,
)
from luicode.providers.mistral import MistralProvider
from luicode.providers.mistral.reasoning import normalize_mistral_stream
from luicode.providers.nvidia_nim import NvidiaNimProvider
from luicode.providers.nvidia_nim.native_tool_stream import (
...
async def _harness(kind, handler):
    ...
    try:
        with (
            ...
            patch(
                "luicode.providers.openai_chat.provider.create_chat_client",
                return_value=client,
            ),
        ):
...
async def test_mistral_cleanup_preserves_normalization_error():
    ...
    try:
        with (
            patch(
                "luicode.providers.mistral.reasoning.normalize_mistral_chunk",
                side_effect=original,
            ),
            patch("luicode.providers.http.trace_event") as trace,
