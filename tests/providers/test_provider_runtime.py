...
from luicode.providers.runtime import (
    ProviderRuntime,
...
def test_importing_runtime_does_not_eager_load_other_adapters() -> None:
    ...
    code = (
        "import sys\n"
        "import luicode.providers.runtime\n"
        "assert 'luicode.providers.open_router' not in sys.modules\n"
    )
    proc = subprocess.run(
...
async def test_abandoned_creation_failure_has_no_unretrieved_exception():
    ...
    try:
        ...
        # It must not also report a task exception that LUICODE failed to retrieve.
        assert all(
