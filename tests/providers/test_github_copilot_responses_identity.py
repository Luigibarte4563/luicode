...
from luicode.core.history_replay import decode_replay
from luicode.core.json_types import JsonObject
from luicode.core.openai_responses import OpenAIResponsesRequest
from luicode.providers.github_copilot.types import CopilotEgress
...
def _input(responses: bool, model: str) -> MessagesRequest | OpenAIResponsesRequest:
    ...
    if responses:
        return OpenAIResponsesRequest.model_validate(
            {
                "model": model,
                "input": "Call luicode_echo with value LUICODE_TOOL_OK.",
                "tools": [
                    {"type": "function", "name": "luicode_echo", "parameters": schema}
                ],
            }
        )
    return MessagesRequest.model_validate(
        {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "Call luicode_echo with value LUICODE_TOOL_OK.",
                }
            ],
            "tools": [{"name": "luicode_echo", "input_schema": schema}],
        }
    )
...
async def test_captured_copilot_tool_identity_survives_stream_and_result_continuation(
    ...
    try:
        ...
        if responses:
            ...
            assert starts == [
                {"type": "tool_use", "id": call_id, "name": "luicode_echo", "input": {}}
            ]
            fragments = [
                delta["partial_json"]
                for event in events
            ...
            assert arguments == {"value": "LUICODE_TOOL_OK"}
            followup = MessagesRequest.model_validate(
                {
                    "model": harness.runtime.name,
                    "messages": [
                        {"role": "user", "content": "Call luicode_echo."},
                        {
                            "role": "assistant",
                            "content": [
                                {
        ...
        harness.responses_content = responses_sse(
            _text_delta("LUICODE_TOOL_OK"), _completed_event()
        ).encode()
        ...
        assert calls[0]["name"] == "luicode_echo"
        assert json.loads(calls[0]["arguments"]) == {"value": "LUICODE_TOOL_OK"}
        assert results[0]["output"] == "LUICODE_TOOL_OK"
        if responses:
            assert calls == output
...
async def test_copilot_interleaved_tools_use_output_positions_and_preserve_final_replay_payloads(
    ...
    for number, fragment in enumerate(('{"value":', '"LUICODE_TOOL_OK"}')):
        events.extend(
            {
        ...
        events.append(
            {
                ...
                "item": {
                    **item,
                    ...
                    "status": "completed",
                    "arguments": '{"value":"LUICODE_TOOL_OK"}',
                },
            }
        )
    events.append(
