...
from luicode.config.provider_catalog import GEMINI_DEFAULT_BASE
...
def test_build_request_body_preserves_caller_extra_body(gemini_provider):
    # This opaque sentinel verifies LUICODE's pass-through contract only; it does not
...
def test_build_request_body_strips_unsupported_metadata_key(gemini_provider):
    ...
    entry or LUICODE would otherwise forward it verbatim via the SDK merge.
    """
...
def test_gemini_rejects_caller_thinking_config_with_luicode_reasoning_control(
    gemini_provider: GeminiProvider,
) -> None:
