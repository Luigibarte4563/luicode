...
from luicode.config.nim import NimSettings
from luicode.config.provider_catalog import NVIDIA_NIM_DEFAULT_BASE
from luicode.core.failures import ExecutionFailure
from luicode.core.history_replay import (
    ReplayOrigin,
    ReplayRecord,
    encode_replay,
)
from luicode.core.openai_responses.models import OpenAIResponsesRequest
from luicode.core.reasoning import ReasoningEffort, ReasoningPolicy
from luicode.providers.admission import UPSTREAM_TRANSIENT_TOTAL_ATTEMPTS
from luicode.providers.nvidia_nim import NvidiaNimProvider
from luicode.providers.nvidia_nim.client import _PROFILE as NIM_PROFILE
from luicode.providers.nvidia_nim.tool_schema import (
    NIM_TOOL_ARGUMENT_ALIASES_KEY,
)
from luicode.providers.stream_recovery import RecoveryHoldbackBuffer
            ...
            (
                {
                    ...
                    "function": {
                        "name": "Grep",
                        "arguments": '{"pattern":"needle","_luicode_arg_',
                    },
                },
                None,
            ),
...
async def test_argument_aliases_survive_shared_history_correction(early_sse):
    ...
    def provider_factory():
        with patch(
            "luicode.providers.nvidia_nim.client._PROFILE",
...
async def test_init(provider_config):
    ...
    with patch("luicode.providers.openai_chat.client.AsyncOpenAI") as mock_openai:
...
def test_responses_request_uses_nim_chat_policy(provider_config):
    ...
    assert body[NIM_TOOL_ARGUMENT_ALIASES_KEY] == {
        original_name: {"_luicode_arg_type": "type"}
    }
    parameters = function["parameters"]
    assert isinstance(parameters, dict)
    assert parameters["properties"] == {"_luicode_arg_type": {"type": "string"}}
    ...
    req = make_request(
        messages=[
            ...
            message(
                ...
                [
                    block(
                        type="tool_result", tool_use_id=tool_id, content="LUICODE_206"
                    ),
...
async def test_native_minimax_tool_markup_restores_nim_argument_aliases(nim_provider):
    ...
    raw = (
        ...
        f"{namespace}<_luicode_arg_type>py{namespace}</_luicode_arg_type>"
        f"{namespace}</invoke>"
        f"{namespace}</tool_call>"
    )
...
async def test_midstream_native_tool_suffix_failure_recovers_without_duplication(
    ...
    with (
        ...
        patch(
            "luicode.providers.stream_recovery.RecoveryHoldbackBuffer",
...
async def test_stream_messages_restores_aliased_tool_arguments(nim_provider):
    ...
    mock_chunk = _tool_call_chunk(
        name="Grep",
        arguments=json.dumps({"pattern": "needle", "-A": 2, "_luicode_arg_type": "py"}),
    )
    ...
    assert "_luicode_arg_A" not in properties
    assert "_luicode_arg_type" in properties

    deltas = _input_json_deltas(events)
    assert len(deltas) == 1
    assert json.loads(deltas[0]) == {"pattern": "needle", "-A": 2, "type": "py"}
    assert "_luicode_arg_type" not in deltas[0]
...
async def test_stream_messages_buffers_chunked_aliased_tool_arguments(nim_provider):
    ...
    second_chunk = _tool_call_chunk(
        name=None,
        arguments='"_luicode_arg_type": "py"}',
        tool_id="call_chunked",
    )
...
async def test_stream_messages_restores_nested_aliased_tool_arguments(nim_provider):
    ...
    mock_chunk = _tool_call_chunk(
        ...
        arguments=json.dumps(
            {"parent": {"_luicode_arg_type": "page_id", "id": "page_123"}}
        ),
    )
