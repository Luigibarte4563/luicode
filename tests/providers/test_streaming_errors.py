...
from luicode.core.openai_tool_names import OpenAIToolNameCodec
...
from luicode.providers.openai_chat.tool_calls import (
    OpenAIToolCallAssembler,
    OpenAIToolCallCollector,
    iter_heuristic_tool_use_events,
)
from luicode.providers.openai_chat.transport import (
...
class TestStreamingExceptionHandling:
    ...
    async def test_function_tag_tool_stream_becomes_one_anthropic_tool_use(self):
        ...
        assert json.loads(input_json) == {"command": "printf LUICODE_STEP_TOOL"}
    ...
    async def test_primary_replay_and_continuation_share_five_attempts(self):
        ...
        with (
            ...
            patch("luicode.providers.admission.trace_event") as attempt_trace,
        ):
            events = await _collect_stream(provider, request)
            ...
            _make_tool_assembler(provider).process_tool_call(
                {
                    ...
                    "function": {
                        ...
                        "arguments": '{"_luicode_arg_type":"file"}',
                    },
                },
                sse,
                tool_names=codec,
                tool_name_buffers={},
                tool_argument_aliases={original: {"_luicode_arg_type": "type"}},
                tool_argument_alias_buffers={},
            )
        )
...
async def test_tool_argument_mapping_survives_correction_then_stream_reopen(collector):
    ...
    async with _harness("chat", responder, chat_provider_factory=_alias_provider) as (
        ...
        assert all("_luicode_nim_tool_argument_aliases" not in body for body in bodies)
