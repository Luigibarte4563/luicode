"""LUICODE opt-in and interactive login over isolated native-account test doubles."""
...
from luicode.providers.github_copilot.login import DeviceChallenge
...
async def test_login_timeout_cleans_up_and_reports_local_deadline(
    ...
    assert "LUICODE's time limit" in (manager.status().message or "")
    await manager.close()
