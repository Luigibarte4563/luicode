...
from luicode.config.provider_catalog import VERTEX_AI_API_ROOT
...
from luicode.providers.model_listing import ModelListResponseError
from luicode.providers.vertex import VertexProvider
from luicode.providers.vertex.auth import GoogleAccessTokenProvider
from luicode.providers.vertex.endpoint import (
    vertex_openai_base_url,
    vertex_publisher_models_url,
    vertex_service_endpoint,
)
from luicode.providers.vertex.models import extract_vertex_model_page
...
def test_vertex_provider_supplies_renewable_token_callback_to_openai() -> None:
    ...
    with (
        patch("luicode.providers.openai_chat.client.AsyncOpenAI") as openai_client,
        patch("luicode.providers.vertex.client.httpx.AsyncClient"),
    ):
...
def test_vertex_rejects_caller_thinking_config_with_luicode_reasoning_control() -> None:
    provider = _provider()
    request = make_messages_request(
        "google/gemini",
        thinking=None,
