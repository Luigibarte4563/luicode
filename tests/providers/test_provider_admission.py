...
from luicode.providers.failure_policy import ProviderRecoveryExhausted
...
async def test_attempt_outcome_and_close_are_idempotent() -> None:
    ...
    with patch("luicode.providers.admission.trace_event") as trace:
        attempt = await _open(execution)
...
async def test_direct_exhaustion_trace_keeps_the_logical_request_id() -> None:
    ...
    with (
        patch("luicode.providers.admission.trace_event") as trace,
...
async def test_openai_413_does_not_retry_sleep_or_open_recovery() -> None:
    ...
    with (
        patch("luicode.providers.admission.asyncio.sleep") as sleep,
        patch("luicode.providers.admission.trace_event") as trace,
...
async def test_one_leader_backs_off_while_followers_coalesce() -> None:
    ...
    with patch(
        "luicode.providers.admission.asyncio.sleep",
