...
def test_anthropic_post_routes_accept_x_api_key(app):
    ...
    with (
        patch("luicode.api.routes.get_token_count", return_value=1),
        patch(
            "luicode.api.routes._create_messages_response",
...
def test_proxy_auth_token_normalizes_configured_whitespace(app):
    ...
    with patch("luicode.api.routes.get_token_count", return_value=3):
        r = client.post(
            "/v1/messages/count_tokens",
            json=payload,
