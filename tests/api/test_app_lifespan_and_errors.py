...
from luicode.providers.nvidia_nim.client import NvidiaNimProvider
from luicode.providers.nvidia_nim.voice import NvidiaNimTranscriber
from luicode.runtime.application import (
    ApplicationRuntime,
    startup_failure_message,
)
from luicode.runtime.asgi import RuntimeASGIApp
from luicode.runtime.bootstrap import _create_transcriber, build_asgi_app
...
def _redirect_luicode_home(monkeypatch, tmp_path):
    home = tmp_path / "home"
...
def test_application_error_handler_does_not_log_error_message():
    ...
    with patch("luicode.api.app.logger.error") as log_error:
...
def test_bootstrap_configures_default_log_and_publishes_only_services(tmp_path):
    ...
    with (
        patch(
            "luicode.runtime.bootstrap.server_log_path",
            return_value=log_path,
        ),
        patch("luicode.runtime.bootstrap.configure_logging") as configure,
    ):
...
def test_bootstrap_app_transparently_exposes_fastapi_interface() -> None:
    with patch("luicode.runtime.bootstrap.configure_logging"):
...
def test_bootstrap_wires_the_codex_catalog_publisher() -> None:
    ...
    with (
        patch("luicode.runtime.bootstrap.configure_logging"),
        patch(
            "luicode.runtime.bootstrap.CodexModelCatalogPublisher",
...
def test_bootstrap_honors_process_log_file_override(monkeypatch, tmp_path):
    ...
    with patch("luicode.runtime.bootstrap.configure_logging") as configure:
