...
from luicode.providers.opencode import create_opencode_provider
...
async def wire_client(provider_id="opencode_zen", handler=None):
    ...
    with patch("luicode.providers.openai_chat.client.AsyncOpenAI", side_effect=sdk):
    ...
    app = create_test_app(
        Settings(
            ...
            proxy_auth_token="luicode-private-token",
        ),
    ...
    try:
        async with httpx.AsyncClient(
            ...
            base_url="http://luicode.test",
            headers={"Authorization": "Bearer luicode-private-token"},
        ) as client:
...
async def test_unusable_user_agent_keeps_fallback_after_another_client_request(
    ...
    async with wire_client(provider_id) as (client, provider, requests, _catalogs):
        ...
        headers = [
            (b"Authorization", b"Bearer luicode-private-token"),
        ...
        response = await client.send(
            httpx.Request(
                "POST",
                "http://luicode.test/v1/responses",
                headers=headers,
                json=request,
            )
        )
            ...
            for conversation in ("conversation-a", "conversation-a", "conversation-b"):
                response = await client.post(
                    ...
                    headers=[
                        ...
                        (b"X-Other-Launch-Id", b"launcher-fallback"),
                ...
                assert "x-luicode-launch-id" not in upstream.headers
        assert {
            name: value
            for name, value in provider._client.default_headers.items()
            if isinstance(value, str)
    ...
    async with wire_client() as (client, _provider, requests, _catalogs):
        ...
        response = await client.post(
            ...
            headers={
                ...
                "x-luicode-launch-id": "launcher-fallback",
    ...
    async with wire_client() as (client, _provider, requests, _catalogs):
        for launch_id, native_id in (
            ...
            response = await client.post(
                ...
                headers={"x-luicode-launch-id": launch_id, "session-id": native_id},
            )
            assert response.status_code == 200, response.text
            assert requests[-1].headers["x-opencode-session"] == (
                native_id or launch_id
            )
            assert "x-luicode-launch-id" not in requests[-1].headers
