...
from luicode.application.model_metadata import (
    ProviderModelInfo,
    ProviderModelRefreshResult,
)
from luicode.config.admin.values import MASKED_SECRET
from luicode.config.provider_catalog import PROVIDER_CATALOG
from luicode.config.server_urls import local_admin_url
...
def test_admin_retirement_preview_apply_and_runtime_agree(monkeypatch, tmp_path):
    ...
    env_file = tmp_path / ".luicode" / ".env"
    env_file.parent.mkdir(parents=True)
    env_file.write_text(
        "LUICODE_CONFIG_SCHEMA=1\nMODEL=groq/default\nMODEL_OPUS=groq/managed\n"
...
def _offline_credential_checks(monkeypatch):
    ...
    monkeypatch.setattr(
        "luicode.runtime.application.check_credentials",
...
def test_admin_page_uses_installed_version(monkeypatch, tmp_path):
    _set_home(monkeypatch, tmp_path)
    monkeypatch.setattr(
        "luicode.api.admin_routes.package_version",
        lambda: "9.8.7",
    )
    ...
    assert 'src="/admin/assets/9.8.7/luicode-wordmark-dark.svg"' in response.text
...
def test_admin_versioned_wordmark_serves_packaged_file(monkeypatch, tmp_path):
    asset_path = (
        Path(__file__).resolve().parents[2]
        / "src"
        / "luicode"
    ...
    _set_home(monkeypatch, tmp_path)
    response = _local_client(create_test_app()).get(
        f"/admin/assets/{package_version()}/luicode-wordmark-dark.svg"
    )
...
def test_admin_unexpected_errors_are_never_cached(monkeypatch, tmp_path):
    ...
    with patch(
        "luicode.runtime.configuration.ConfigurationService.admin_config",
...
def test_admin_api_fetches_bypass_browser_cache():
    script = Path("src/luicode/api/admin_static/admin.js").read_text(encoding="utf-8")
...
def test_admin_static_model_combobox_owns_dropdown_and_search_behavior():
    script = Path("src/luicode/api/admin_static/admin.js").read_text(encoding="utf-8")
    combobox_script = Path("src/luicode/api/admin_static/model_combobox.js").read_text(
        encoding="utf-8"
    )
    styles = Path("src/luicode/api/admin_static/admin.css").read_text(encoding="utf-8")

    assert 'api("/admin/api/models" + (refresh ? "/refresh" : "")' in script
    assert 'field.type === "model" || field.type === "optional_model"' in script
    assert "new window.LuicodeModelCombobox" in script
    ...
    assert "class LuicodeModelCombobox" in combobox_script
...
def test_admin_config_masks_secrets_and_exposes_manifest(monkeypatch, tmp_path):
    ...
    assert "LUICODE_OPEN_BROWSER" in keys
    ...
    assert body["paths"] == {"managed": str(tmp_path / ".luicode" / ".env")}
    catalog_smoke_keys = {
        f"LUICODE_SMOKE_MODEL_{provider_id.upper()}" for provider_id in PROVIDER_CATALOG
    }
    actual_smoke_keys = {
        field["key"]
        for field in body["fields"]
        if field["key"].startswith("LUICODE_SMOKE_MODEL_")
    }
    assert actual_smoke_keys == catalog_smoke_keys | {
        "LUICODE_SMOKE_MODEL_MISTRAL_REASONING"
    }
    open_browser_field = next(
        field for field in body["fields"] if field["key"] == "LUICODE_OPEN_BROWSER"
    )
...
def test_admin_apply_persists_open_browser_for_next_launch(monkeypatch, tmp_path):
    ...
    response = _local_client(app).post(
        "/admin/api/config/apply",
        json={"values": {"LUICODE_OPEN_BROWSER": False}},
    )
    ...
    managed_env = tmp_path / ".luicode" / ".env"
    assert "LUICODE_OPEN_BROWSER=false" in managed_env.read_text(encoding="utf-8")
...
def test_admin_apply_preserves_hidden_diagnostics_and_smoke_values(
    ...
    env_file.write_text(
        "\n".join(
            [
                ...
                "LUICODE_SMOKE_MODEL_ZAI=zai/smoke-model",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    assert "LUICODE_SMOKE_MODEL_ZAI=zai/smoke-model" in text
...
def test_restart_signal_failure_reports_saved_config_and_manual_restart(
    ...
    assert "PORT=9090" in (tmp_path / ".luicode" / ".env").read_text()
...
def test_admin_never_reads_arbitrary_current_directory_env(monkeypatch, tmp_path):
    ...
    assert response.status_code == 200
    managed_text = (tmp_path / ".luicode" / ".env").read_text("utf-8")
...
def test_admin_apply_preserves_false_and_numeric_zero(monkeypatch, tmp_path):
    ...
    assert response.status_code == 200
    assert response.json()["applied"] is True
    managed = (tmp_path / ".luicode" / ".env").read_text(encoding="utf-8")
    assert "LUICODE_OPEN_BROWSER=false" in managed
...
def test_admin_local_provider_status_reports_reachable(monkeypatch, tmp_path):
    ...
    with patch("luicode.api.admin_routes.httpx.AsyncClient", FakeAsyncClient):
...
def test_admin_local_provider_status_checks_all_providers_concurrently(
    ...
    with patch("luicode.api.admin_routes.httpx.AsyncClient", SlowAsyncClient):
        response = _local_client(app).get("/admin/api/providers/local-status")
...
def test_admin_local_provider_failure_does_not_return_exception_text(
    ...
    with patch(
        "luicode.api.admin_routes.httpx.AsyncClient",
