...
import luicode.runtime.web_tools.constants as web_tool_constants
from luicode.api.handlers import MessagesHandler
...
from luicode.application.routing import (
    ModelRouter,
    ProviderModelTarget,
    ResolvedModelRoute,
    RoutedMessagesRequest,
)
from luicode.application.web_tools.ports import (
    WebFetchEgressPolicy,
    WebFetchEgressViolation,
)
from luicode.application.web_tools.request import (
...
from luicode.config.reasoning import ReasoningPreference
from luicode.config.settings import Settings
from luicode.core.anthropic.models import (
    ContentBlockServerToolUse,
    Message,
    MessagesRequest,
    Tool,
)
from luicode.core.anthropic.stream_contracts import (
...
from luicode.messaging.event_parser import parse_cli_event
from luicode.runtime.web_tools import egress as web_egress
from luicode.runtime.web_tools.client import (
    HTTPWebToolsClient,
    _drain_response_body_capped,
    _read_response_body_capped,
)
from luicode.runtime.web_tools.egress import enforce_web_fetch_egress
...
def test_web_tool_user_agent_reports_installed_package_version() -> None:
    assert {
        "User-Agent": (f"Mozilla/5.0 compatible; luicode/{package_version()}")
...
async def test_service_rejects_forced_server_tool_when_local_handler_is_disabled(
    ...
    """Every provider needs LUICODE's local handler for forced server tools."""
...
async def test_web_fetch_pins_validated_addresses_for_every_redirect(monkeypatch):
    ...
    with (
        patch(
            "luicode.runtime.web_tools.client.ClientSession",
            return_value=client_cm,
        ),
        patch(
            "luicode.runtime.web_tools.client.TCPConnector",
...
async def test_run_web_fetch_excess_redirects_raises():
    ...
    with (
        patch("luicode.runtime.web_tools.constants._MAX_WEB_FETCH_REDIRECTS", 1),
        patch(
            "luicode.runtime.web_tools.client.ClientSession",
            return_value=client_cm,
        ),
...
async def test_streams_web_fetch_error_summary_generic_by_default(monkeypatch):
    ...
    with patch("luicode.application.web_tools.service.logger.warning") as log_warn:
        raw = "".join(
            [
                event
                async for event in _local_tool_body(
