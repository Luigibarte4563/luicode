...
from luicode.api.handlers import (
    MessagesHandler,
    ResponsesHandler,
    TokenCountHandler,
)
...
async def test_messages_handler_normalizes_safety_classifier_policy(
    ...
    with patch("luicode.api.handlers.messages.trace_event") as trace_mock:
        response = await handler.create(request)
        assert isinstance(response, StreamingResponse)
    ...
    assert _trace_events(trace_mock, "luicode.api.route.safety_classifier_policy") == [
        {
            "stage": "routing",
            "event": "luicode.api.route.safety_classifier_policy",
            "source": "api",
            "model": "nvidia_nim/test-model",
...
async def test_messages_handler_preserves_thinking_for_non_classifier() -> None:
    ...
    assert (
        _trace_events(
            trace_mock,
            "luicode.api.route.safety_classifier_policy",
        )
        == []
    )
...
async def test_messages_handler_tolerates_required_thinking_for_classifier() -> None:
    ...
    request = MessagesRequest(
        model="claude-3-luicode-no-thinking/nvidia_nim/test-model",
        max_tokens=100,
        stream=True,
...
def test_token_count_handler_routes_and_counts_tokens() -> None:
    ...
    with patch("luicode.api.handlers.token_count.trace_event") as trace:
