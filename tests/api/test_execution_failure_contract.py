...
from luicode.core.anthropic import MessagesRequest
...
def _terminal_trace(trace_mock: MagicMock) -> dict[str, Any]:
    return dict(
        next(
            ...
            == "luicode.api.response.terminal_execution_error"
        )
    )
...
def test_grouped_pre_start_execution_failure_keeps_canonical_wire_error(
    ...
    with (
        resolver_patch,
        patch("luicode.api.response_streams.trace_event") as trace_mock,
        client,
    ):
        response = client.post(path, json=payload)
