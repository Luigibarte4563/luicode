...
from luicode.config.constants import DEFAULT_MODEL
...
@pytest.mark.parametrize("prefix", ["", "anthropic/", "claude-3-luicode-no-thinking/"])
def test_retired_model_ingress_calls_default_provider(client, stream, prefix):
    ...
    if prefix == "claude-3-luicode-no-thinking/":
...
def client(app):
    ...
    with (
        patch(
            "luicode.api.routes.resolve_provider",
...
def test_create_message_rejects_unportable_image_as_invalid_request() -> None:
    with patch(
        "luicode.providers.openai_chat.client.AsyncOpenAI",
    ...
    with (
        patch("luicode.api.routes.resolve_provider", return_value=provider),
...
def test_create_message_pre_start_provider_error_returns_terminal_json(
    ...
    with (
        patch("luicode.api.response_streams.trace_event") as trace,
        patch("luicode.application.execution.trace_event") as execution_trace,
    ):
        response = client.post("/v1/messages", json=payload)
    ...
    route_trace = next(
        ...
        if call.kwargs.get("event") == "luicode.api.route.resolved"
    )
    assert route_trace["request_id"] == request_id
    terminal_trace = next(
        call.kwargs
        for call in trace.call_args_list
        if call.kwargs.get("event") == "luicode.api.response.terminal_execution_error"
    )
    assert terminal_trace == {
        "stage": "egress",
        "event": "luicode.api.response.terminal_execution_error",
        "source": "api",
        "wire_api": "messages",
