...
from luicode.core.gateway_model_ids import desktop_model_id
...
def test_pending_disconnect_survives_runtime_restart(monkeypatch):
    ...
    profile = root / "configLibrary" / f"{desktop.LUICODE_ID}.json"
    unlink = Path.unlink
...
def test_desktop_model_view_preserves_labels_and_query_precedence():
    ...
    with TestClient(create_test_app(settings)) as client:
        headers = {"X-LUICODE-Model-View": "claude-desktop"}
        ...
        assert (
            client.get(
                "/v1/models", headers={"X-LUICODE-Model-View": "invalid"}
            ).status_code
            == 422
        )
...
def test_desktop_token_count_routes_to_original_provider(no_thinking):
    ...
    with (
        ...
        patch("luicode.api.routes.get_token_count", return_value=7),
