...
from luicode.config import env_migrations
from luicode.config.env_files import dotenv_values_from_file
from luicode.config.env_migrations import (
...
@pytest.mark.parametrize("schema", ["", "LUICODE_CONFIG_SCHEMA=1\n"])
...
def test_retirement_atomic_write_failure_preserves_managed_file(monkeypatch, tmp_path):
    managed, _ = _paths(monkeypatch, tmp_path)
    managed.parent.mkdir(parents=True)
    original = b"LUICODE_CONFIG_SCHEMA=1\nMODEL=github_models/old\n"
...
def test_fresh_install_writes_only_schema_metadata(
    ...
    assert dotenv_values_from_file(managed) == {
        "LUICODE_CONFIG_SCHEMA": CONFIG_SCHEMA_VERSION
    }
...
def test_explicit_file_overlays_once_and_then_becomes_inert(
    ...
    consolidate_managed_config({"LUICODE_ENV_FILE": str(explicit)})
    ...
    result = consolidate_managed_config({"LUICODE_ENV_FILE": str(explicit)})
...
def test_post_schema_invalid_explicit_path_is_never_dereferenced(
    ...
    managed, _legacy = _paths(monkeypatch, tmp_path)
    managed.parent.mkdir(parents=True)
    managed.write_text("LUICODE_CONFIG_SCHEMA=1\n", encoding="utf-8")

    result = consolidate_managed_config(
        {"LUICODE_ENV_FILE": str(tmp_path / "missing.env")}
    )
...
def test_pre_schema_invalid_explicit_path_fails_without_writing(
    ...
    with pytest.raises(FileNotFoundError, match=r"missing\.env"):
        consolidate_managed_config({"LUICODE_ENV_FILE": str(tmp_path / "missing.env")})
...
def test_explicit_managed_path_is_deduplicated(
    ...
    result = consolidate_managed_config({"LUICODE_ENV_FILE": str(managed)})
...
def test_fresh_install_ignores_process_token(monkeypatch, tmp_path):
    ...
    assert settings.proxy_auth_token == "luicode"
...
def test_only_managed_schema_marker_is_trusted(
    ...
    managed, legacy = _paths(monkeypatch, tmp_path)
    legacy.parent.mkdir(parents=True)
    legacy.write_text("LUICODE_CONFIG_SCHEMA=99\nMODEL=deepseek/imported\n")

    consolidate_managed_config({"LUICODE_CONFIG_SCHEMA": "99"})

    values = dotenv_values_from_file(managed)
    assert values["LUICODE_CONFIG_SCHEMA"] == "1"
...
def test_concurrent_loaders_produce_one_valid_managed_file(
    ...
    assert dotenv_values_from_file(managed)["LUICODE_CONFIG_SCHEMA"] == "1"
    assert list(managed.parent.glob("*.tmp")) == []
...
def test_newer_managed_schema_fails_closed(
    ...
    managed, _legacy = _paths(monkeypatch, tmp_path)
    managed.parent.mkdir(parents=True)
    managed.write_text("LUICODE_CONFIG_SCHEMA=2\n", encoding="utf-8")
