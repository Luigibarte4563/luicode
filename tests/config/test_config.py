...
from luicode.config import loader
from luicode.config.constants import (
...
from luicode.config.model_refs import (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("LUICODE_OPEN_BROWSER", "open_admin_browser", "false", False),
...
def test_process_auth_token_cannot_override_retained_default(
    ...
    assert snapshot.settings.proxy_auth_token == "luicode"
...
def test_paths_are_owned_by_luicode_home(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("USERPROFILE", str(tmp_path))

    assert messaging_state_dir_path() == tmp_path / ".luicode" / "agent_workspace"
    assert server_log_path() == tmp_path / ".luicode" / "logs" / "server.log"
