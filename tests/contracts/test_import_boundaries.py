...
_PACKAGE_ROOT = _REPO_ROOT / "src" / "luicode"
...
FACADE_ONLY_BOUNDARIES = {
    "luicode.core.openai_responses",
    "luicode.messaging.trees",
...
OPTIONAL_IMPORT_OWNERS = {
    ...
    "riva": "luicode.providers.nvidia_nim.voice",
}
...
def test_api_configuration_access_goes_through_runtime() -> None:
    storage_modules = {
        "luicode.config.loader",
        "luicode.config.env_files",
        "luicode.config.env_migrations",
    }
    offenders = [
        record.describe()
        for record in _scan_imports(_PACKAGE_ROOT)
        if record.importer.startswith("luicode.api.")
...
def test_legacy_first_party_import_detector_rejects_bare_owner_names() -> None:
    record = ImportRecord(
        importer="luicode.api.routes",
        imported="core.anthropic",
        path="luicode/api/routes.py",
        line=7,
        inside_function=False,
    )

    assert _legacy_first_party_import_offenders([record], {"api", "core"}) == [
        "luicode/api/routes.py:7: luicode.api.routes -> core.anthropic"
    ]
...
def test_core_does_not_import_provider_transport_sdks() -> None:
    ...
    offenders = [
        ...
        for record in _scan_imports(_PACKAGE_ROOT)
        if (
            record.importer == "luicode.core"
            or record.importer.startswith("luicode.core.")
        )
...
def test_web_tool_workflow_has_no_http_adapter_owner() -> None:
    former_owner = "luicode.api.web_tools"
    application_owner = "luicode.application.web_tools"
...
def test_runtime_imports_without_optional_voice_dependencies() -> None:
    ...
    script = "\n".join(
        (
            ...
            "import luicode.runtime.bootstrap",
            "import luicode.api.app",
        )
    )
...
def test_supported_messaging_facade_is_explicit() -> None:
    import luicode.messaging as facade
    from luicode.messaging.managed_protocols import (
        ManagedClaudeSessionManagerProtocol,
        ManagedClaudeSessionProtocol,
    )
    from luicode.messaging.models import IncomingMessage, MessageScope
    from luicode.messaging.platforms.ports import OutboundMessenger
...
def test_message_tree_mutability_stays_behind_its_facade() -> None:
    import luicode.messaging.trees as facade
