"""Behavior contracts for LUICODE's standalone native-Windows Muse lifecycle."""
...
def test_muse_installer_validates_owner_record(tmp_path: Path, powershell: str) -> None:
    record_path = tmp_path / ".luicode-muse-install.json"
    record_path.write_text(
        json.dumps(
            {
                "schema_version": 1,
                "owner": "luicode-muse-installer",
    ...
    assert result.returncode == 0, result.stderr
    assert json.loads(result.stdout)["owner"] == "luicode-muse-installer"
...
def _managed_paths(local_app_data: Path) -> tuple[Path, Path, Path]:
    ...
    return root, root / "bin" / "muse.exe", root / ".luicode-muse-install.json"
...
def test_muse_installer_rejects_file_at_managed_root_before_fetch(
    ...
    assert result.returncode != 0
    assert "not owned by LUICODE" in result.stderr
...
def _create_managed_install(
    ...
    *,
    ...
    (root / f".luicode-muse-install.json.{residue_id}.tmp").write_bytes(b"temporary")
    (root / f".luicode-muse-install.json.{residue_id}.backup").write_bytes(b"backup")
    if unknown_file:
        ...
        (root / ".luicode-muse-install.json.user.tmp").write_bytes(b"keep")
        (root / f".LUICODE-MUSE-INSTALL.JSON.{near_match_id}.backup").write_bytes(
            b"keep"
        )
...
def test_muse_uninstaller_preserves_unknown_files_and_nonempty_root(
    ...
    assert (root / ".luicode-muse-install.json.user.tmp").read_bytes() == b"keep"
    assert (
        root / ".LUICODE-MUSE-INSTALL.JSON.fedcba9876543210fedcba9876543210.backup"
    ).read_bytes() == b"keep"
...
def test_muse_uninstaller_is_idempotent_when_managed_install_is_absent(
    ...
    assert result.returncode == 0, result.stderr
    assert "No LUICODE-managed Muse Code installation" in result.stdout
    assert not local_app_data.exists()
