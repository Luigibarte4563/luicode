...
def _assert_uv_ready_without_luicode_install(calls: list[str]) -> None:
    assert "uv:--version" in calls
    assert not any("--refresh-package luicode" in call for call in calls)
...
def _posix_command(name: str, *, version_output: str | None = None) -> str:
    ...
    return f"""#!/bin/sh
echo "{name}:$*" >> "$CALL_LOG"
if [ "{name}" = "opencode" ] && [ "${{LUICODE_RUNNING_PHASE:-}}" = "opencode-plugin" ]; then
    if [ -e "$LUICODE_PROCESS_MARKER.probed" ]; then
        : > "$LUICODE_PROCESS_MARKER"
    else
        : > "$LUICODE_PROCESS_MARKER.probed"
    fi
fi
...
if [ "${{1:-}}" = "--version" ]; then
    if [ "${{LUICODE_RUNNING_PHASE:-}}" = "late" ]; then
        : > "$LUICODE_PROCESS_MARKER"
    fi
...
if [ "${{1:-}}" = "tool" ] && [ "${{2:-}}" = "install" ]; then
    ...
    if [ "$FAIL_STEP" = "luicode-install" ]; then
        exit 33
    fi
    mkdir -p "$tool_bin"
    cp "$FAKE_FIXTURES/luicode-command.sh" "$tool_bin/luicode-server"
    cp "$FAKE_FIXTURES/luicode-command.sh" "$tool_bin/luicode-desktop"
    cp "$FAKE_FIXTURES/luicode-command.sh" "$tool_bin/luicode-claude"
    cp "$FAKE_FIXTURES/luicode-command.sh" "$tool_bin/luicode-pi"
    cp "$FAKE_FIXTURES/luicode-command.sh" "$tool_bin/luicode-opencode"
    cp "$FAKE_FIXTURES/luicode-command.sh" "$tool_bin/luicode-cline"
    cp "$FAKE_FIXTURES/luicode-command.sh" "$tool_bin/luicode-hermes"
    cp "$FAKE_FIXTURES/luicode-command.sh" "$tool_bin/luicode-dsh"
    cp "$FAKE_FIXTURES/luicode-command.sh" "$tool_bin/luicode-grok"
    cp "$FAKE_FIXTURES/luicode-command.sh" "$tool_bin/luicode-muse"
    cp "$FAKE_FIXTURES/luicode-command.sh" "$tool_bin/luicode-aider"
    cp "$FAKE_FIXTURES/luicode-command.sh" "$tool_bin/luicode-update"
    if [ "$FAIL_STEP" != "luicode-missing" ]; then
        cp "$FAKE_FIXTURES/luicode-command.sh" "$tool_bin/luicode-codex"
    fi
    chmod +x "$tool_bin"/luicode-*
    exit 0
fi
...
""",
    ...
    def run_interactive(
        ...
        *args: str,
        ...
        env = self.env | {
            "FAIL_STEP": fail_step,
            "LUICODE_INSTALLER": str(installer),
        }
        command = [
            "/bin/sh",
            "-c",
            'cat "$LUICODE_INSTALLER" | /bin/sh -s -- "$@"',
            "luicode-installer",
            *args,
        ]
        return subprocess.run(
...
def posix_harness(tmp_path: Path) -> PosixHarness:
    ...
    _write_executable(
        bin_dir / "pgrep",
        """#!/bin/sh
[ -n "${LUICODE_RUNNING_COMMAND:-}" ] || exit 1
if [ "${LUICODE_RUNNING_PHASE:-early}" != "early" ] && [ ! -e "$LUICODE_PROCESS_MARKER" ]; then
    exit 1
fi
case "$*" in
    "-x $LUICODE_RUNNING_COMMAND"|"-f (^|/)$LUICODE_RUNNING_COMMAND([[:space:]]|$)") printf '4242\n'; exit 0 ;;
    *) exit 1 ;;
esac
""",
    )
    _write_executable(
        ...
        """#!/bin/sh
[ "${1:-}" = "passwd" ] || exit 61
printf 'luicode-test:x:1000:1000::%s:/bin/sh\n' "$FAKE_INFERRED_HOME"
""",
    )
    _write_executable(
        bin_dir / "curl",
...
case "$url" in
    ...
    *opencode.ai*)
        ...
        if [ "${LUICODE_RUNNING_PHASE:-}" = "opencode-download" ]; then
            : > "$LUICODE_PROCESS_MARKER"
        fi
        ;;
...
""",
    ...
    _write_executable(
        fixtures / "luicode-command.sh",
        """#!/bin/sh
name=${0##*/}
echo "$name:$*" >> "$CALL_LOG"
if [ "$FAIL_STEP" = "luicode-verify" ]; then
    exit 36
fi
if [ "$name" = "luicode-desktop" ] && [ "${1:-}" = "--export-icon" ]; then
    [ "$FAIL_STEP" = "desktop-icon-export" ] && exit 37
    mkdir -p "$(dirname "$2")"
    printf 'fake icon\n' > "$2"
fi
if [ "$name" = "luicode-server" ] && [ "${1:-}" = "--version" ]; then
    echo "luicode 3.5.18"
fi
""",
    )
    ...
    env.update(
        {
            ...
            "LUICODE_PROCESS_MARKER": str(tmp_path / "luicode-process-ready"),
            "LUICODE_RUNNING_COMMAND": "",
            "LUICODE_RUNNING_PHASE": "early",
            "FAKE_UNAME": "Linux",
...
def test_install_sh_auto_selects_installed_harnesses(
    ...
    assert "for luicode-" not in result.stdout
...
def test_install_sh_asks_only_about_missing_harnesses(
    ...
    assert result.returncode == 0, result.stdout
    assert re.findall(r"for luicode-(\w+)\?", result.stdout) == [
        "codex",
        "pi",
        "cline",
        "hermes",
...
def test_install_sh_does_not_use_uv_install_directory_for_aider(
    ...
    assert result.returncode == 0, result.stdout
    assert "Install Aider for luicode-aider?" in result.stdout
...
def test_install_sh_installs_selected_hermes_without_setup(
    ...
    assert "Run Hermes Agent with: luicode-hermes" in result.stdout
    assert not any("hermes:setup" in call for call in calls)
...
def test_install_sh_stops_when_selected_hermes_install_fails(
    ...
    _assert_uv_ready_without_luicode_install(posix_harness.calls())
...
def test_install_sh_accepts_aider_as_the_only_selected_agent(
    ...
    assert calls.index(aider_install) < calls.index("aider:--version")
    assert not any("aider.chat/install" in call for call in calls)
    assert "Run Aider with: luicode-aider" in result.stdout
...
def test_install_sh_installs_selected_dsh_at_exact_preview(
    ...
    assert "Run DeepSeek Harness with: luicode-dsh" in result.stdout
    assert not any(call.startswith("rtk:init") for call in calls)
...
def test_install_sh_noninteractive_skips_dsh_without_node(
    ...
    assert result.returncode == 0, result.stderr
    assert (
        "luicode-dsh wrapper is ready after you install DeepSeek Harness"
        in result.stdout
    )
...
def test_install_sh_reprompts_then_installs_only_selected_agent(
    ...
    assert "Run Codex with: luicode-codex" in result.stdout
    assert "Run Claude Code with: luicode-claude" not in result.stdout
    assert "Run Pi with: luicode-pi" not in result.stdout
    assert "Run OpenCode with: luicode-opencode" not in result.stdout
    assert "Run Cline with: luicode-cline" not in result.stdout
    calls = posix_harness.calls()
...
def test_install_sh_creates_native_macos_app_and_desktop_link(
    ...
    assert result.returncode == 0, result.stderr
    app = posix_harness.root / "home" / "Applications" / "luicode.app"
    plist = app / "Contents" / "Info.plist"
    owner_file = app / "Contents" / ".luicode-owner"
    launcher = app / "Contents" / "MacOS" / "luicode-desktop"
    icon = app / "Contents" / "Resources" / "AppIcon.icns"
    desktop_link = posix_harness.root / "home" / "Desktop" / "luicode.app"
    assert owner_file.read_text(encoding="utf-8").strip() == ("com.luicode.desktop")
    ...
    expected_command = str(tool_bin / "luicode-desktop").replace("'", "'\\''")
    ...
    assert any(
        call == f"luicode-desktop:--export-icon {icon}"
...
def test_install_sh_stops_if_macos_icon_export_fails(
    ...
    assert not (posix_harness.root / "home" / "Desktop" / "luicode.app").exists()
...
def test_install_sh_rejects_unowned_macos_app_bundle(
    ...
    assert result.returncode != 0
    assert "not managed by luicode" in result.stderr
    assert plist.read_text(encoding="utf-8") == "foreign app"
    assert not (contents / ".luicode-owner").exists()
...
def test_install_sh_preserves_unrelated_macos_desktop_link(
    ...
    unrelated.mkdir()
    desktop_link = desktop / "luicode.app"
    desktop_link.symlink_to(unrelated, target_is_directory=True)

    result = posix_harness.run()

    assert result.returncode == 0, result.stderr
    assert "non-LUICODE link" in result.stdout
...
def test_install_sh_continues_when_pi_is_not_installed(
    ...
    assert "uv-install" in calls
    assert "luicode-server:--version" in calls
...
def test_install_sh_stops_without_success_on_each_failure(
    ...
    forbidden = {
        ...
        "luicode-install": "uv:tool update-shell",
        "luicode-missing": "luicode-server:--version",
    }.get(failure)
...
def test_install_sh_rejects_broken_existing_client_without_replacing_it(
    ...
    assert result.returncode != 0
    calls = posix_harness.calls()
    _assert_uv_ready_without_luicode_install(calls)
    assert not any("claude.ai" in call for call in calls)
...
def test_install_sh_rejects_running_luicode_before_mutation(
    posix_harness: PosixHarness,
    command_name: str,
) -> None:
    posix_harness.env["LUICODE_RUNNING_COMMAND"] = command_name
...
def test_install_sh_rechecks_for_luicode_process_before_tool_replacement(
    posix_harness: PosixHarness,
) -> None:
    posix_harness.add_client("claude")
    posix_harness.add_client("codex")
    posix_harness.add_client("pi")
    posix_harness.add_uv("0.12.13")
    posix_harness.env["LUICODE_RUNNING_COMMAND"] = "luicode-server"
    posix_harness.env["LUICODE_RUNNING_PHASE"] = "late"

    result = posix_harness.run()

    assert result.returncode != 0
    assert "luicode-server (PID 4242)" in result.stderr
    assert not any(
        "--refresh-package luicode" in call for call in posix_harness.calls()
    )
...
def test_install_sh_ignores_similarly_named_process(
    ...
    posix_harness.env["LUICODE_RUNNING_COMMAND"] = "luicode-server-helper"
...
@pytest.mark.parametrize(
    ...
    (
        ("luicode", "/home/user/.local/bin/luicode"),
        ("luicode-server", "/usr/bin/python3 /home/user/.local/bin/luicode-server"),
    ),
)
...
def test_install_ps1_waits_for_gui_icon_export() -> None:
    ...
    body = _braced_body(installer, "function Export-LuicodeDesktopIcon")
...
def test_install_ps1_gui_icon_export_completes_before_returning(
    ...
    installed_desktop_command = Path(sys.executable).with_name("luicode-desktop.exe")
    desktop_command = tmp_path / "icon-exporter.exe"
    shutil.copy2(installed_desktop_command, desktop_command)
    destination = tmp_path / "profile with spaces" / ".luicode" / "app-icon.ico"
    env = os.environ | {
        "LUICODE_TEST_DESKTOP_COMMAND": str(desktop_command),
        "LUICODE_TEST_ICON_PATH": str(destination),
    }
    script = "\n".join(
        (
            ...
            (
                ...
                "-DesktopCommand $env:LUICODE_TEST_DESKTOP_COMMAND "
                "-IconPath $env:LUICODE_TEST_ICON_PATH"
            ),
        )
    )
    ...
    assert (
        ...
        == (_repo_root() / "src" / "luicode" / "assets" / "app-icon.ico").read_bytes()
    )
...
def _create_windows_shortcut(
    ...
    env = os.environ | {
        "LUICODE_TEST_SHORTCUT": str(shortcut_path),
        "LUICODE_TEST_TARGET": str(target_path),
    }
    subprocess.run(
        [
            powershell,
            ...
            (
                ...
                "$shortcut = $shell.CreateShortcut($env:LUICODE_TEST_SHORTCUT); "
                "$shortcut.TargetPath = $env:LUICODE_TEST_TARGET; "
                "$shortcut.Save()"
            ),
        ],
        check=True,
...
def _windows_shortcut_icon(
    ...
    process_env = env | {"LUICODE_TEST_SHORTCUT": str(shortcut_path)}
    completed = subprocess.run(
        [
            powershell,
            "-NoProfile",
...
def _batch_client(name: str, *, version_output: str | None = None) -> str:
    ...
    return f"""@echo off
echo {name}:%*>>"%CALL_LOG%"
if "{name}"=="opencode" if "%LUICODE_RUNNING_PHASE%"=="opencode-plugin" (
    if exist "%LUICODE_PROCESS_MARKER%.probed" (
        type nul > "%LUICODE_PROCESS_MARKER%"
    ) else (
        type nul > "%LUICODE_PROCESS_MARKER%.probed"
    )
)
...
exit /b 59
:version
if "%LUICODE_RUNNING_PHASE%"=="late" type nul > "%LUICODE_PROCESS_MARKER%"
if "%FAIL_STEP%"=="uv-verify" exit /b 52
echo uv {version}
exit /b 0
:install
if "%5"=="python3.12" goto install_aider
if "%FAIL_STEP%"=="luicode-install" exit /b 53
if not exist "%UV_BIN_DIR%" mkdir "%UV_BIN_DIR%"
copy /y "%FAKE_FIXTURES%\luicode-command.cmd" "%UV_BIN_DIR%\luicode-server.cmd" >nul
copy /y "%FAKE_FIXTURES%\luicode-command.cmd" "%UV_BIN_DIR%\luicode-desktop.cmd" >nul
copy /y "%FAKE_FIXTURES%\luicode-command.cmd" "%UV_BIN_DIR%\luicode-claude.cmd" >nul
copy /y "%FAKE_FIXTURES%\luicode-command.cmd" "%UV_BIN_DIR%\luicode-pi.cmd" >nul
copy /y "%FAKE_FIXTURES%\luicode-command.cmd" "%UV_BIN_DIR%\luicode-opencode.cmd" >nul
copy /y "%FAKE_FIXTURES%\luicode-command.cmd" "%UV_BIN_DIR%\luicode-cline.cmd" >nul
copy /y "%FAKE_FIXTURES%\luicode-command.cmd" "%UV_BIN_DIR%\luicode-hermes.cmd" >nul
copy /y "%FAKE_FIXTURES%\luicode-command.cmd" "%UV_BIN_DIR%\luicode-dsh.cmd" >nul
copy /y "%FAKE_FIXTURES%\luicode-command.cmd" "%UV_BIN_DIR%\luicode-grok.cmd" >nul
copy /y "%FAKE_FIXTURES%\luicode-command.cmd" "%UV_BIN_DIR%\luicode-muse.cmd" >nul
copy /y "%FAKE_FIXTURES%\luicode-command.cmd" "%UV_BIN_DIR%\luicode-aider.cmd" >nul
copy /y "%FAKE_FIXTURES%\luicode-command.cmd" "%UV_BIN_DIR%\luicode-update.cmd" >nul
if not "%FAIL_STEP%"=="luicode-missing" copy /y "%FAKE_FIXTURES%\luicode-command.cmd" "%UV_BIN_DIR%\luicode-codex.cmd" >nul
exit /b 0
:install_aider
...
""",
    ...
    def run_interactive(
        ...
        self.env["LUICODE_INSTALLER_ANSWERS"] = json.dumps({"answers": answers})
...
def powershell_harness(
    ...
    (fixtures / "luicode-command.cmd").write_text(
        """@echo off
for %%I in ("%~f0") do set "LUICODE_NAME=%%~nI"
echo %LUICODE_NAME%:%*>>"%CALL_LOG%"
if "%FAIL_STEP%"=="luicode-verify" exit /b 55
if "%LUICODE_NAME%"=="luicode-desktop" if "%1"=="--export-icon" if "%FAIL_STEP%"=="desktop-icon-export" exit /b 56
if "%LUICODE_NAME%"=="luicode-desktop" if "%1"=="--export-icon" (
    if not exist "%~dp2" mkdir "%~dp2"
    echo fake icon>"%~2"
)
if "%LUICODE_NAME%"=="luicode-server" if "%1"=="--version" echo luicode 3.5.18
exit /b 0
""",
        encoding="utf-8",
    )
...
if ($env:LUICODE_TEST_USER_PATH) {
    $env:Path = "$originalPath$([IO.Path]::PathSeparator)$env:LUICODE_TEST_USER_PATH"
}
...
""",
    ...
    for asset_name in (
        ...
        with zipfile.ZipFile(
            ...
            archive.write(
                Path(sys.executable).with_name("luicode-server.exe"),
...
function Invoke-RestMethod {
    ...
    if ($env:LUICODE_RUNNING_PHASE -eq "opencode-download" -and $Uri.Contains("opencode.ai/files/bin/")) {
        New-Item -ItemType File -Path $env:LUICODE_PROCESS_MARKER -Force | Out-Null
    }
}
function Get-Process {
    ...
    if ([string]::IsNullOrWhiteSpace($env:LUICODE_RUNNING_COMMAND)) {
        return
    }
    if (
        $env:LUICODE_RUNNING_PHASE -ne "early" -and
        -not (Test-Path -LiteralPath $env:LUICODE_PROCESS_MARKER)
    ) {
        return
    }
    foreach ($requestedName in $Name) {
        if ($requestedName -eq $env:LUICODE_RUNNING_COMMAND) {
            [pscustomobject] @{ Id = 4242; ProcessName = $requestedName }
        }
    }
}
$installerSource = [IO.File]::ReadAllText($env:LUICODE_INSTALLER)
$installerSource = $installerSource.Replace(
    '[Environment]::GetEnvironmentVariable("Path", "User")',
    '$env:LUICODE_TEST_USER_PATH'
)
if ($env:LUICODE_INSTALLER_ANSWERS) {
    $script:InstallerAnswers = (ConvertFrom-Json $env:LUICODE_INSTALLER_ANSWERS).answers
...
$fakeArchiveVersionProbe = @'
    $output = if ([IO.Path]::GetExtension($OpenCodePath) -eq ".exe") {
        ...
        if ($env:LUICODE_RUNNING_PHASE -eq "opencode-publish" -and [IO.File]::ReadAllText($OpenCodePath) -ne "old-opencode-v1") {
            New-Item -ItemType File -Path $env:LUICODE_PROCESS_MARKER -Force | Out-Null
        }
    ...
    env.update(
        {
            ...
            "FAKE_TOOL_BIN": str(tool_bin),
            "FAKE_NPM_PREFIX": str(npm_prefix),
            "LUICODE_INSTALLER": str(_repo_root() / "scripts" / "install.ps1"),
            "LUICODE_PROCESS_MARKER": str(tmp_path / "luicode-process-ready"),
            "LUICODE_RUNNING_COMMAND": "",
            "LUICODE_RUNNING_PHASE": "early",
...
def test_install_sh_discovery_preserves_existing_command_priority(
    ...
    assert result.returncode == 0, result.stdout
    assert f"for luicode-{candidate}?" not in result.stdout
    calls = posix_harness.calls()
    assert "claude:--version" in calls
...
def _assert_setup_preserves_other_commands(
    ...
    assert any("--refresh-package luicode" in call for call in calls)
...
def test_install_ps1_verifies_new_luicode_after_custom_tool_discovery(
    powershell_harness: PowerShellHarness,
) -> None:
    powershell_harness.add_uv("0.12.13")
    powershell_harness.env["UV_TOOL_BIN_DIR"] = str(powershell_harness.tool_bin)
    _write_executable(
        powershell_harness.bin_dir / "luicode-server.cmd",
        _batch_client("stale-luicode"),
    )
    ...
    calls = powershell_harness.calls()
    assert "luicode-server:--version" in calls
    assert not any(call.startswith("stale-luicode:") for call in calls)
...
def _assert_lookup_is_isolated(
    ...
    harness.env["LUICODE_TEST_USER_PATH"] = str(tool_bin)
...
def test_install_ps1_discovers_muse_from_user_path(
    ...
    powershell_harness.env["LUICODE_TEST_USER_PATH"] = str(user_bin)

    result = powershell_harness.run_interactive(["n"] * 8)

    assert result.returncode == 0, result.stderr
    assert "for luicode-muse?" not in result.stdout
    calls = powershell_harness.calls()
...
def test_install_ps1_fresh_install_is_verified(
    ...
    icon = home / ".luicode" / "app-icon.ico"
    assert icon.read_text(encoding="utf-8").strip() == "fake icon"
    assert calls[-1] == f'luicode-desktop:--export-icon "{icon}"'
    desktop_shortcut = home / "Desktop" / "luicode.lnk"
    ...
    assert (
        app_data / "Microsoft" / "Windows" / "Start Menu" / "Programs" / "luicode.lnk"
    ).is_file()
...
def test_install_ps1_stops_if_windows_icon_export_fails(
    ...
    home = Path(powershell_harness.env["USERPROFILE"])
    assert not (home / "Desktop" / "luicode.lnk").exists()
...
@pytest.mark.parametrize("command_name", LUICODE_COMMANDS)
def test_install_ps1_rejects_running_luicode_before_mutation(
    powershell_harness: PowerShellHarness,
    command_name: str,
) -> None:
...
def test_install_ps1_rechecks_for_luicode_process_before_tool_replacement(
    powershell_harness: PowerShellHarness,
) -> None:
    powershell_harness.add_client("claude")
    powershell_harness.add_client("codex")
    powershell_harness.add_client("pi")
    powershell_harness.add_uv("0.12.13")
    powershell_harness.env["LUICODE_RUNNING_COMMAND"] = "luicode-server"
    powershell_harness.env["LUICODE_RUNNING_PHASE"] = "late"
    ...
    assert "luicode-server (PID 4242)" in result.stderr
    assert not any(
        "--refresh-package luicode" in call for call in powershell_harness.calls()
    )
...
def test_installers_use_native_clients_and_single_python_selection() -> None:
    ...
    for text in (shell, powershell):
        for command_name in LUICODE_COMMANDS:
    ...
    if isinstance(harness, PowerShellHarness):
        subprocess.run(
            [
                ...
                "-NoProfile",
                "-Command",
                "New-Item -ItemType Junction -Path $env:LUICODE_TEST_LINK -Target $env:LUICODE_TEST_TARGET -ErrorAction Stop | Out-Null",
            ],
            env=harness.env
            | {"LUICODE_TEST_LINK": str(linked), "LUICODE_TEST_TARGET": str(shared)},
            capture_output=True,
            text=True,
            check=True,
        )
...
def test_install_sh_opencode_running_blocks_native_migration(
    ...
    original = binary.read_bytes()
    posix_harness.env["LUICODE_RUNNING_COMMAND"] = "opencode"
    result = posix_harness.run()
    assert result.returncode != 0
    assert "Close OpenCode" in result.stderr
    assert binary.read_bytes() == original
...
def test_install_ps1_opencode_running_blocks_native_migration(
    ...
    original = binary.read_bytes()
    powershell_harness.env["LUICODE_RUNNING_COMMAND"] = "opencode"
    result = powershell_harness.run()
    assert result.returncode != 0
    assert "Close OpenCode" in result.stderr
    assert binary.read_bytes() == original
...
def _assert_external_opencode_is_not_shadowed(
    ...
    if interactive:
        ...
        assert "for luicode-opencode?" not in result.stdout
    else:
        result = harness.run()
    assert result.returncode != 0
