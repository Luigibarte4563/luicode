...
def posix_uninstall_harness(tmp_path: Path) -> PosixUninstallHarness:
    ...
    luicode_home = home / ".luicode"
    log = tmp_path / "calls.log"
    for path in (bin_dir, tool_bin, luicode_home):
        path.mkdir(parents=True)
    (luicode_home / "config.json").write_text("{}", encoding="utf-8")
    for name in LUICODE_COMMANDS:
...
case "$*" in
    *"$LUICODE_RUNNING_COMMAND"*) printf '4242\n'; exit 0 ;;
    *) exit 1 ;;
esac
""",
    )
...
if [ "${1:-}" = "tool" ] && [ "${2:-}" = "uninstall" ]; then
    ...
    if [ "$FAIL_STEP" = "missing" ] || [ "$FAIL_STEP" = "stale-entrypoint" ]; then
        echo 'Tool `luicode` is not installed' >&2
        exit 2
    fi
    for name in luicode-desktop luicode-server luicode-claude luicode-codex luicode-pi luicode-opencode luicode-cline luicode-hermes luicode-dsh luicode-grok luicode-muse luicode-aider luicode-update luicode-init luicode; do
        /bin/rm -f "$FAKE_TOOL_BIN/$name"
    done
    echo "Uninstalled luicode"
    exit 0
fi
exit 43
""",
    ...
    _write_executable(
        ...
        """#!/bin/sh
echo "rm:$*" >> "$CALL_LOG"
if [ "$FAIL_STEP" = "purge" ] && [ "$*" = "-rf $HOME/.luicode" ]; then
...
""",
    ...
    app = home / "Applications" / "luicode.app"
    contents = app / "Contents"
    contents.mkdir(parents=True)
    (contents / ".luicode-owner").write_text(
        "com.luicode.desktop\n",
        encoding="utf-8",
    )
    desktop = home / "Desktop"
    desktop.mkdir()
    (desktop / "luicode.app").symlink_to(app, target_is_directory=True)
    ...
    return PosixUninstallHarness(home, bin_dir, tool_bin, luicode_home, log, env)


def test_uninstall_sh_removes_and_verifies_only_luicode(
    posix_uninstall_harness: PosixUninstallHarness,
) -> None:
    result = posix_uninstall_harness.run()

    assert result.returncode == 0, result.stderr
    assert "luicode has been removed and verified." in result.stdout
    assert not posix_uninstall_harness.luicode_home.exists()
    assert all(
    ...
    assert posix_uninstall_harness.calls() == [
        ...
        f"rm:-f {posix_uninstall_harness.home / 'Desktop' / 'luicode.app'}",
        f"rm:-rf {posix_uninstall_harness.home / 'Applications' / 'luicode.app'}",
        f"rm:-rf {posix_uninstall_harness.luicode_home}",
    ]
...
def test_uninstall_sh_preserves_unrelated_macos_desktop_link(
    ...
    desktop_link = posix_uninstall_harness.home / "Desktop" / "luicode.app"
    desktop_link.unlink()
...
def test_uninstall_sh_preserves_unowned_macos_app_bundle(
    ...
    app = posix_uninstall_harness.home / "Applications" / "luicode.app"
...
def test_uninstall_sh_preserves_config_when_tool_removal_is_unconfirmed(
    ...
    assert posix_uninstall_harness.luicode_home.exists()
    assert "luicode has been removed and verified." not in result.stdout
...
def test_uninstall_sh_dry_run_is_non_mutating(
    ...
    assert posix_uninstall_harness.luicode_home.exists()
    assert all(
        (posix_uninstall_harness.tool_bin / name).exists() for name in LUICODE_COMMANDS
    )
    assert posix_uninstall_harness.calls() == []
...
def test_uninstall_sh_rejects_running_luicode_before_mutation(
    posix_uninstall_harness: PosixUninstallHarness,
    command_name: str,
) -> None:
    posix_uninstall_harness.env["LUICODE_RUNNING_COMMAND"] = command_name
...
def powershell_uninstall_harness(
    ...
    log = tmp_path / "calls.log"
    for path in (bin_dir, tool_bin, luicode_home, app_data, local_app_data):
        path.mkdir(parents=True)
    (luicode_home / "config.json").write_text("{}", encoding="utf-8")
    for name in LUICODE_COMMANDS:
    ...
    uv_commands = " ".join(LUICODE_COMMANDS)
...
if "%FAIL_STEP%"=="missing" echo Tool `luicode` is not installed 1>&2 & exit /b 2
if "%FAIL_STEP%"=="stale-entrypoint" echo Tool `luicode` is not installed 1>&2 & exit /b 2
...
function Remove-Item {
    ...
    if (
        ...
        $LiteralPath -eq (Join-Path $env:USERPROFILE ".luicode")
    ) {
...
$installer = [scriptblock]::Create([IO.File]::ReadAllText($env:LUICODE_UNINSTALLER))
...
""",
    ...
    env.update(
        {
            ...
            "CALL_LOG": str(log),
            "FAKE_TOOL_BIN": str(tool_bin),
            "LUICODE_UNINSTALLER": str(_repo_root() / "scripts" / "uninstall.ps1"),
            "LUICODE_RUNNING_COMMAND": "",
            "FAIL_STEP": "",
            "UNINSTALL_DRY_RUN": "0",
        }
    ...
    return PowerShellUninstallHarness(
        home, bin_dir, tool_bin, luicode_home, log, env, powershell, wrapper
    )


def test_uninstall_ps1_removes_and_verifies_only_luicode(
    ...
    assert result.returncode == 0, result.stderr
    assert "luicode has been removed and verified." in result.stdout
    assert not powershell_uninstall_harness.luicode_home.exists()
    assert all(
    ...
    assert powershell_uninstall_harness.calls() == [
        "uv:tool dir --bin",
        "uv:tool uninstall luicode",
        f"remove:{Path(powershell_uninstall_harness.env['USERPROFILE']) / 'Desktop' / 'luicode.lnk'}",
        f"remove:{Path(powershell_uninstall_harness.env['APPDATA']) / 'Microsoft' / 'Windows' / 'Start Menu' / 'Programs' / 'luicode.lnk'}",
        f"remove:{powershell_uninstall_harness.luicode_home}",
    ]
...
def test_uninstall_ps1_preserves_config_when_tool_removal_is_unconfirmed(
    ...
    assert result.returncode != 0
    assert powershell_uninstall_harness.luicode_home.exists()
    assert "luicode has been removed and verified." not in result.stdout
    assert not any(
...
@pytest.mark.parametrize("command_name", LUICODE_COMMANDS)
def test_uninstall_ps1_rejects_running_luicode_before_mutation(
    powershell_uninstall_harness: PowerShellUninstallHarness,
    command_name: str,
) -> None:
    powershell_uninstall_harness.env["LUICODE_RUNNING_COMMAND"] = command_name
...
def test_uninstallers_guard_running_commands_and_preserve_shared_owners() -> None:
    ...
    for text in (shell, powershell):
        for command in LUICODE_COMMANDS:
