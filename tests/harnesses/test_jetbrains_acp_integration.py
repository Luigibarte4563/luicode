...
from luicode.harnesses import jetbrains_acp_integration as jb
from luicode.harnesses.jetbrains_acp_integration import (
    registry_path,
    system_root,
)
...
def test_lifecycle_preserves_other_agents_and_refreshes_owned_settings(files):
    ...
    entry = saved["agent_servers"]["Claude Code (LUICODE)"]
    ...
    refreshed = json.loads(config.read_text())["agent_servers"]["Claude Code (LUICODE)"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "[]",
        '{"agent_servers":[]}',
        '{"agent_servers":{"Claude Code (LUICODE)":{"command":"other"}}}',
...
def test_platform_layout_and_launch(
    ...
    node, script = install(registry, systems)
    jb.configure(config, "http://localhost:8082", "token", True)
    entry = json.loads(config.read_text())["agent_servers"]["Claude Code (LUICODE)"]
    assert entry["command"] == str(node)
...
def test_failed_atomic_replace_preserves_existing_config(files, monkeypatch):
    from luicode.harnesses import config_file
