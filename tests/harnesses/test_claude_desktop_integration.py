...
from luicode.config.paths import claude_desktop_disconnect_path
from luicode.harnesses import claude_desktop_integration as desktop
from luicode.harnesses.claude_desktop_integration import (
    check_unmanaged,
    config_root,
)
...
def profile(root):
    return root / "configLibrary" / f"{desktop.LUICODE_ID}.json"
...
def test_fresh_connection_and_disconnect_preserve_native_library(tmp_path):
    ...
    assert read(meta(root))["appliedId"] == desktop.LUICODE_ID
...
def test_refresh_repairs_registered_profile_header_without_changing_selection(tmp_path):
    ...
    assert read(profile(tmp_path))["inferenceCustomHeaders"] == {
        "X-Other": "preserved",
        "X-LUICODE-Model-View": "claude-desktop",
    }
...
def test_disconnect_recovers_every_failure_boundary(tmp_path, monkeypatch, failure):
    ...
    write(
        meta(tmp_path),
        {
            "appliedId": desktop.LUICODE_ID,
            "entries": [{"id": desktop.LUICODE_ID, "name": "LUICODE"}],
        },
    )
    ...
    def fail_write(path, content):
        ...
        step = (
            ...
            if any(e["id"] == desktop.LUICODE_ID for e in value.get("entries", []))
            else "metadata"
        )
        if step == failure:
...
def test_disconnect_recovers_orphan_after_delete_failure(
    ...
    assert read(mode(tmp_path))["deploymentMode"] == "1p"
    assert read(meta(tmp_path))["appliedId"] != desktop.LUICODE_ID
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {},
        {"entries": []},
        {"entries": [{"id": desktop.LUICODE_ID, "name": "LUICODE"}], "appliedId": []},
        {"entries": [{"id": desktop.LUICODE_ID, "name": "LUICODE"}], "appliedId": {}},
        {
...
def test_retry_preserves_a_later_external_choice(tmp_path, monkeypatch, change):
    ...
    desktop.configure(tmp_path, URL, TOKEN, False, disconnect_path=record)
    expected = metadata | {
        "entries": [e for e in metadata["entries"] if e["id"] != desktop.LUICODE_ID]
    }
...
def test_pending_removal_handles_missing_profile_and_later_luicode_selection(tmp_path):
    record = claude_desktop_disconnect_path()
    desktop.configure(tmp_path, URL, TOKEN, True, disconnect_path=record)
