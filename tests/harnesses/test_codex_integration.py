...
from luicode.harnesses import codex_integration
from luicode.harnesses.codex_integration import config_path
...
def test_connect_disconnect_preserve_model_comments_and_other_providers(tmp_path):
    ...
    assert data["model_provider"] == "luicode"
    assert Path(data["model_catalog_json"]) == tmp_path / "catalog.json"
    assert data["model_providers"]["luicode"] == {
        "name": "luicode",
        "base_url": URL + "/v1",
        "wire_api": "responses",
        "auth": {"command": "luicode-codex", "args": ["--print-proxy-auth-token"]},
    }
...
def test_inline_tables_merge_without_losing_extra_values(tmp_path):
    path = tmp_path / "config.toml"
    path.write_text(
        'model_providers = {luicode = {name = "Old", extra = 7, auth = {command = "old", keep = true}}, other = {name = "Other"}} # Keep comment\n'
    )
    operate(path, True)
    data = tomllib.loads(path.read_text())
    assert data["model_providers"]["luicode"]["extra"] == 7
    assert data["model_providers"]["luicode"]["auth"]["keep"] is True
...
def test_manual_connection_ignores_model_and_display_name(tmp_path):
    path = tmp_path / "config.toml"
    path.write_text(
        'model = "another-model"\nmodel_provider = "luicode"\n'
        f'model_catalog_json = "{(tmp_path / "catalog.json").as_posix()}"\n'
        '[model_providers.luicode]\nname = "My LUICODE"\nbase_url = "http://localhost:8082/v1/"\nwire_api = "responses"\n'
        '[model_providers.luicode.auth]\ncommand = "luicode-codex"\nargs = ["--print-proxy-auth-token"]\n'
    )
...
@pytest.mark.parametrize(
    ...
    [
        "model_providers = {}\n",
        'model_providers = {luicode = {name = "Old"}}\n',
        '[model_providers.luicode]\nname = "Old"\n[other]\nkeep = true\n[model_providers.other]\nname = "Other"\n[model_providers.luicode.auth]\ncommand = "old"\n',
    ],
)
def test_valid_table_layouts_can_be_updated(tmp_path, content):
    ...
    assert operate(path, True)["connected"] is True
    assert tomllib.loads(path.read_text())["model_providers"]["luicode"]["auth"][
        "args"
...
@pytest.mark.parametrize(
    "old,new",
    [
        ('model_provider = "luicode"', 'model_provider = "other"'),
        ('wire_api = "responses"', 'wire_api = "chat"'),
        (
            'base_url = "http://127.0.0.1:8082/v1"',
            'base_url = "http://127.0.0.1:8083/v1"',
        ),
        ('command = "luicode-codex"', 'command = "other"'),
...
def test_disconnect_keeps_global_selections_for_another_provider(tmp_path):
    ...
    path.write_text(
        path.read_text()
        .replace('model_provider = "luicode"', 'model_provider = "other"')
    ...
    assert "luicode" not in data.get("model_providers", {})
