        ...
        "data": [
            {
                "id": model_id,
                "provider_model_ref": (
                    model_id.removeprefix("claude-3-luicode-no-thinking/")
                ),
...
def test_codex_catalog_preserves_no_thinking_only_entries_for_routing() -> None:
    catalog = build_codex_model_catalog(
        catalog_models_from_response(
            _models_payload("claude-3-luicode-no-thinking/open_router/plain-model")
        )
    )

    assert _slugs(catalog) == ["claude-3-luicode-no-thinking/open_router/plain-model"]
