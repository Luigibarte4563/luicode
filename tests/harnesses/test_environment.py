from luicode.harnesses.environment import with_local_proxy_bypass
...
def test_child_proxy_bypass_preserves_existing_proxy_policy() -> None:
    ...
    env = with_local_proxy_bypass(
        base_env,
        proxy_root_url="http://luicode.internal:8082",
    )

    assert env["HTTP_PROXY"] == "http://proxy.example:3128"
    assert env["KEEP_ME"] == "yes"
    assert env["NO_PROXY"] == (
        "example.com,localhost,10.0.0.0/8,127.0.0.1,::1,luicode.internal"
    )
