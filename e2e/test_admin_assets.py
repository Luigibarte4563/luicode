...
def test_admin_removes_chat_drafts_and_preserves_other_storage(page, admin_base_url):
    page.add_init_script("""(() => {
      for (let index = 0; index < 16; index++) {
        sessionStorage.setItem(`luicode.chat.draft.${index}`, `old draft ${index}`);
      }
      for (let index = 0; index < 16; index++) {
        sessionStorage.setItem(`luicode.code.${index}`, `keep code draft ${index}`);
      }
      sessionStorage.setItem('luicode.chat.draftWithoutDot', 'keep unrelated key');
    ...
    assert page.evaluate("Object.fromEntries(Object.entries(sessionStorage))") == {
        **{f"luicode.code.{index}": f"keep code draft {index}" for index in range(16)},
        "luicode.chat.draftWithoutDot": "keep unrelated key",
...
def test_admin_loads_current_release_assets_before_rendering_dynamic_content(
    ...
    assert f"{versioned_root}/luicode-wordmark-dark.svg" in requested_paths
